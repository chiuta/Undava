...
function qff_live_write($s){ global $DATA; if(!is_dir($DATA.'/live')) @mkdir($DATA.'/live',0775,true); $s['ver']=QFF_SCHEMA; $ok=qff_atomic(qff_live_path($s['code']), json_encode($s, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_PRETTY_PRINT)); if($ok && random_int(1,50)===1) qff_live_sweep(); return $ok; }
...
.tf-ver{font-size:11px;color:var(--muted);opacity:.6;margin-top:2px}
button:focus-visible,a:focus-visible,input:focus-visible,textarea:focus-visible,select:focus-visible,[tabindex]:focus-visible,summary:focus-visible{outline:3px solid var(--accent);outline-offset:2px;border-radius:4px}
.review-screen .rv-list{display:flex;flex-direction:column;gap:8px;margin-top:8px}
.rv-row{display:flex;gap:12px;align-items:flex-start;padding:12px 14px;border-radius:12px;border:1px solid var(--line);background:rgba(255,255,255,.03)}
.rv-row.ok{border-color:rgba(24,189,107,.4);background:rgba(24,189,107,.08)}
.rv-row.no{border-color:rgba(232,56,90,.4);background:rgba(232,56,90,.08)}
.rv-mark{flex:none;width:28px;height:28px;border-radius:50%;display:grid;place-items:center;font-weight:900;color:#fff}
.rv-row.ok .rv-mark{background:var(--good,#18bd6b)}
.rv-row.no .rv-mark{background:var(--bad,#e8385a)}
.rv-q{font-weight:700;line-height:1.4}
.rv-a{font-size:14px;color:var(--muted);margin-top:4px}
.rv-a b{color:#ece7ff;font-weight:800}
#printroot{display:none}
@media print{ .bg,#topbar,#root,#confetti{display:none!important} #printroot{display:block!important} body{background:#fff!important} }
.pq-doc{color:#111;font-family:system-ui,-apple-system,Segoe UI,sans-serif;padding:8px;max-width:720px;margin:0 auto}
.pq-doc h1{font-size:22px;margin:0 0 4px}
.pq-desc{color:#333;margin:0 0 4px;font-size:14px}
.pq-meta{color:#666;font-size:12px;margin:0 0 18px}
.pq-q{margin:0 0 15px;break-inside:avoid;page-break-inside:avoid}
.pq-qh{font-weight:700;font-size:15px;margin:0 0 5px}
.pq-opts{margin:0;padding-left:22px}
.pq-opts li{margin:2px 0}
.pq-ans{font-size:14px;padding-left:4px}
.rv-explain{font-size:13px;color:var(--muted);margin-top:6px;line-height:1.5;background:rgba(255,255,255,.04);padding:8px 10px;border-radius:8px}
.reveal-explain{max-width:520px;margin:14px auto 0;font-size:15px;line-height:1.5;color:#ece7ff;background:rgba(255,255,255,.06);border:1px solid var(--line);padding:12px 16px;border-radius:12px}
.pq-explain{font-size:13px;color:#444;margin-top:4px;padding-left:4px;font-style:italic}
.tf-dot{opacity:.4}
.json-doc{margin:16px 0 0;background:rgba(255,255,255,.03);border:1px solid var(--line);border-radius:var(--r-sm,12px);padding:2px 16px}
.json-doc>summary{cursor:pointer;font-weight:800;font-size:15px;padding:14px 0;list-style:none;color:#ece7ff}
.json-doc>summary::-webkit-details-marker{display:none}
.json-doc>summary::before{content:"▸";color:var(--accent);display:inline-block;width:1.2em}
.json-doc[open]>summary::before{content:"▾"}
.jd-intro{color:var(--muted);font-size:14px;line-height:1.55;margin:0 0 12px}
.jd-actions{display:flex;flex-wrap:wrap;gap:10px;margin:0 0 14px}
.jd-code{background:rgba(0,0,0,.35);border:1px solid var(--line);border-radius:10px;padding:14px 16px;overflow-x:auto;font-family:ui-monospace,Menlo,Consolas,monospace;font-size:12.5px;line-height:1.5;color:#d9d2f5;white-space:pre;margin:0 0 14px}
.jd-ref{color:var(--muted);font-size:13px;line-height:1.65;white-space:pre-wrap;padding:0 0 16px}
...
const I18N = {
  "ro": {
    "print_quiz": "Printează",
    "unsaved_q": "Ai modificări nesalvate. Ieși fără să salvezi?",
    "review_btn": "Recapitulare",
    "review_title": "Recapitulare răspunsuri",
    "review_answer": "Corect:",
    "review_back": "Înapoi la scor",
    "sort_by": "Sortează",
    "sort_default": "Implicit",
    "sort_name": "Nume A-Z",
    "sort_questions": "Nr. întrebări",
    "dup_q": "Duplică întrebarea",
    "lib_search": "Caută un quiz…",
    "lib_nomatch": "Niciun quiz găsit.",
    "tagline": "Quiz fără frontiere",
    "set_sound": "Sunet",
    "settings": "Setări",
    "spin_need": "Adaugă cel puțin 2 elemente.",
    "back_home": "Acasă",
    "spin_title": "Roata norocului",
    "spin_sub": "Introdu nume sau opțiuni, apoi învârte pentru a alege la întâmplare.",
    "spin_go": "Învârte",
    "spin_items": "Elemente",
    "spin_ph": "Un element pe linie…\nAna\nBogdan\nCristina",
    "spin_elim": "Elimină câștigătorul după fiecare tragere",
    "spin_shuffle": "Amestecă",
    "spin_reset": "Resetează",
    "spin_default": "Ana|Bogdan|Cristina|David|Elena|Florin",
    "help_book_eli5": "Ghid simplu",
    "help_book_user": "Manual utilizator",
    "help_book_admin": "Manual administrator",
    "help_title": "Ghid de utilizare",
    "help_sub": "Manual complet, pas cu pas.",
    ...
    "feat_free": "Trade-free",
    "feat_priv": "Zero telemetrie",
    "feat_open": "Un singur fișier",
    "tf_desc": "fără reclame, fără colectare de date, fără cont necesar, fără funcții blocate după plată",
    "tf_tools": "alte unelte",
    ...
    "export": "Exportă",
    "del": "Șterge",
    "lib_title": "Bibliotecă de quiz-uri",
    "lib_sub": "Alege un quiz pentru a-l juca sau edita",
    "lib_mine": "Quiz-urile mele",
    "export_all": "Salvează toate",
    "lib_empty": "Încă nu ai creat niciun quiz. Apasă „Creează quiz” ca să începi.",
    "lib_samples": "Quiz-uri demonstrative",
    "library": "Quiz-urile mele",
    "new_quiz": "Quiz nou",
    "edit_quiz": "Editează quiz-ul",
    "quiz_title": "Titlul quiz-ului",
    "quiz_title_ph": "ex: Cultură generală",
    "quiz_desc": "Descriere (opțional)",
    "quiz_desc_ph": "O scurtă descriere…",
    "questions": "Întrebări",
    "add_question": "Adaugă întrebare",
    "cancel": "Renunță",
    "save": "Salvează quiz-ul",
    "acc_primary_ph": "Răspunsul corect (cel afișat)",
    "acc_alt_ph": "Variantă acceptată (sinonim, altă scriere)",
    "acc_add": "Adaugă variantă acceptată",
    "acc_hint": "Se acceptă și mici greșeli de tipar. Prima variantă e cea afișată ca răspuns corect.",
    "num_target": "Numărul corect",
    "num_target_ph": "ex. 42",
    "num_tol": "Toleranță acceptată",
    ...
    "question_n": (n)=>"Întrebarea "+n,
    "type_quiz": "Grilă (4 variante)",
    "type_tf": "Adevărat / Fals",
    "type_type": "Răspuns liber",
    "type_num": "Numeric (ghicește)",
    "sec": "s",
    "pts_std": "Standard",
    "pts_dbl": "Dublu",
    "question_ph": "Scrie întrebarea aici…",
    "choose_mode": "Mod de joc",
    "mode_solo": "Solo",
    "mode_solo_d": "Joci singur și îți baţi recordul.",
    "mode_hot": "Pe rând",
    "mode_hot_d": "Mai mulți jucători pe același dispozitiv.",
    "shuffle_opt": "Amestecă întrebările și variantele",
    "players": "Jucători",
    "add_player": "+ Adaugă jucător",
    "start_game": "Începe jocul",
    "your_name": "Numele tău",
    "player_ph": "Numele jucătorului",
    "import_copied": "Copiat!",
    "import_title": "Importă un quiz",
    "import_drop": "Trage un fișier .json aici sau atinge pentru a alege",
    "import_paste": "…sau lipește conținutul JSON:",
    "import_paste_ph": "{\"title\":\"…\",\"questions\":[…]}",
    "import_btn": "Importă",
    "import_fmt_title": "Format JSON — creează manual",
    "import_fmt_intro": "Poți scrie un fișier .json cu structura de mai jos și îl imporți aici. Iată un exemplu cu toate tipurile de întrebări:",
    "import_copy": "Copiază exemplul",
    "import_template": "Descarcă șablon",
    "import_fields": "STRUCTURA FIȘIERULUI\n• title — numele quiz-ului\n• desc — descriere scurtă (opțional)\n• color — culoare hex, ex. #2f6bff (opțional)\n• questions — lista de întrebări\n\nFIECARE ÎNTREBARE\n• type — quiz (variante) / tf (adevărat-fals) / type (răspuns scris) / num (numeric)\n• text — enunțul întrebării\n• time — secunde: 5, 10, 20, 30, 60 sau 120\n• points — 1000 (standard) sau 2000 (dublu)\n• answers — răspunsurile (vezi mai jos)\n• tol — doar la num: toleranța acceptată (±)\n• explain — explicație opțională (de ce e corect)\n\nRĂSPUNSURI, DUPĂ TIP\n• quiz: 2–4 variante; marchează corectul cu \"correct\": true\n• tf: 2 variante (Adevărat / Fals); marchează corectul\n• type: 1–6 texte acceptate (toate corecte; adaugă variante de scriere)\n• num: un singur număr; opțional tol pentru toleranță",
    "import_ok": "Quiz importat!",
    "import_err": "Fișier invalid. Verifică formatul JSON.",
    "you": "Tu",
    "need_player": "Adaugă cel puțin un jucător.",
    "scoreboard": "Clasament",
    "q_of": (a,b)=>"Întrebarea "+a+" din "+b,
    "pass_to": "Predă dispozitivul lui",
    "tap_start": "Atinge când ești gata",
    "ready_q": "Pregătit?",
    "num_answer_ph": "Scrie un număr…",
    "type_answer_ph": "Scrie răspunsul…",
    "type_submit": "Trimite",
    "keys_hint": "Apasă 1-4 sau atinge un răspuns",
    "verdict_ok": "Corect!",
    "verdict_no": "Greșit!",
    "verdict_miss": "Fără răspuns",
    "correct_was": "Răspuns corect:",
    "you_wrote": "Ai scris:",
    "streak": (n)=>"Serie de "+n+" 🔥",
    "continue": "Continuă",
    "fastest5": "Cei mai rapizi",
    "stat_pts": "Puncte",
    "stat_correct": "Corecte",
    "stat_acc": "Acuratețe",
    "stat_streak": "Serie max",
    "play_again": "Joacă din nou",
    "podium_title": "Felicitări!",
    "winner_is": "Câștigător",
    "type_need_answer": "Scrie un răspuns.",
    "set_title": "Setări",
    "set_lang": "Limbă",
    "set_lang_d": "Limba interfeței",
    "set_sound_d": "Efecte sonore în timpul jocului",
    ...
    "deleted": "Quiz șters",
    "backup_done": "Backup salvat",
    "quit_q": "Părăsești jocul? Progresul se pierde.",
    "err_title": "Dă un titlu quiz-ului.",
    "err_noq": "Adaugă cel puțin o întrebare.",
    "err_qtext": (n)=>"Întrebarea "+n+" nu are text.",
    "err_acc": (n)=>`Întrebarea ${n}: adaugă cel puțin un răspuns acceptat.`,
    "err_num": (n)=>`Întrebarea ${n}: introdu un număr valid.`,
    "err_ans": (n)=>"Întrebarea "+n+" are nevoie de cel puțin 2 răspunsuri.",
    "err_corr": (n)=>"Marchează răspunsul corect la întrebarea "+n+".",
    "explain_ph": "Explicație (opțional) — de ce e corect răspunsul",
    "net_err": "Eroare de rețea. Verifică conexiunea.",
    "live_t_cloud": "Nor de cuvinte",
    "live_t_cloud_d": "Publicul trimite cuvinte, apar live.",
    "live_t_poll": "Sondaj live",
    "live_t_poll_d": "Vot pe opțiuni, bare în timp real.",
    "live_t_qa": "Întrebări & idei",
    "live_t_qa_d": "Mesaje cu voturi de la public.",
    "live_t_rating": "Rating / NPS",
    "live_t_rating_d": "Notă cu stele sau scor NPS 0–10.",
    "live_t_rank": "Clasament",
    "live_t_rank_d": "Participanții ordonează opțiunile după preferință.",
    "live_t_scale": "Scale (Likert)",
    "live_t_scale_d": "Mai multe afirmații, notate 1–5 (dezacord→acord).",
    "live_t_points": "100 de puncte",
    "live_t_points_d": "Participanții împart un buget de puncte între opțiuni.",
    "cat_quiz": "Chestionare & jocuri",
    "cat_quiz_d": "Grile, adevărat/fals, răspuns liber, numeric · Solo, pe rând, live, temă sau echipe",
    "cat_audience": "Activități de audiență",
    "cat_audience_d": "Colectează răspunsuri live de la public, de pe telefoane. Alege un tip:",
    "cat_deck": "Prezentări",
    "cat_deck_d": "Înlănțuie mai multe activități într-o singură sesiune, navigate în ritmul tău.",
    "deck_build": "Construiește o prezentare",
    "cat_survey": "Chestionar feedback",
    "cat_tools": "Instrumente",
    "live_nav": "În direct",
    "guestbook": "Părerea ta",
    "mode_live": "Live (mulți jucători)",
    "mode_live_d": "Găzduiește pe ecran mare; publicul joacă de pe telefoane.",
    "mode_assign": "Temă (autonom)",
    "mode_assign_d": "Atribuie un quiz; fiecare îl rezolvă singur, în ritm propriu.",
    "team_opt": "Mod pe echipe",
    "team_word": "echipe",
    "assign_note": "Participanții intră de pe telefoane și rezolvă singuri, în ritm propriu. Rezultatele se adună aici.",
    "game_join_note": "Jucătorii se alătură de pe telefoane, scanând codul QR sau introducând codul afișat.",
    "game_host": "Găzduiește live",
    "assign_host": "Atribuie tema",
    "fb_leave": "Lasă un feedback",
    "set_admin": "Administrator",
    "set_admin_on": "Ești autentificat ca administrator.",
    "set_admin_off": "Autentifică-te pentru a edita quiz-urile.",
    "logout": "Ieși",
    "login": "Autentificare",
    "password": "Parolă",
    "password_again": "Repetă parola",
    "fb_need_rating": "Alege o notă (1–5 stele).",
    "fb_need_msg": "Scrie un mesaj.",
    "fb_thanks_mod": "Mulțumim! Mesajul va apărea după aprobare.",
    "fb_thanks": "Mulțumim pentru feedback!",
    "err_pw": "Introdu parola.",
    "admin_ok": "Autentificat!",
    "admin_bad": "Parolă greșită.",
    "err_pw_short": "Parola trebuie să aibă minim 6 caractere.",
    "err_pw_match": "Parolele nu coincid.",
    "admin_out": "Te-ai deconectat.",
    "fb_empty": "Încă niciun feedback. Fii primul!",
    "fb_title": "Părerea vizitatorilor",
    "fb_sub": "Lasă un gând despre quiz-uri și vezi ce au scris alții.",
    "fb_recent": "Feedback recent",
    "fb_local_note": "(salvat local, pe acest dispozitiv)",
    "fb_pick_quiz": "— alege un quiz —",
    "fb_name": "Nume (opțional)",
    "fb_name_ph": "Cum să-ți spunem?",
    "fb_quiz": "Quiz (opțional)",
    "fb_rating": "Notă",
    "fb_msg": "Mesaj",
    "fb_msg_ph": "Ce ți-a plăcut? Ce am putea îmbunătăți?",
    "fb_send": "Trimite feedback",
    "fb_anon": "Anonim",
    "fb_pending": "în așteptare",
    "fb_approve": "Aprobă",
    "qa_pending": "În așteptare",
    "qa_answered": "Răspuns",
    "qa_approve": "Aprobă",
    "qa_star": "Evidențiază",
    "qa_mark": "Marchează ca răspuns",
    "live_show": "Arată",
    "live_hide": "Ascunde",
    "qa_pending_count": (n)=>`${n} în așteptare`,
    "live_waiting": "Se așteaptă răspunsuri…",
    "live_responses": "răspunsuri",
    "rt_prom": "Promotori",
    "rt_pass": "Pasivi",
    "rt_det": "Detractori",
    "rt_avg": "medie",
    "rk_foot": "ordonat după rangul mediu (mai mic = mai bun)",
    "scale_lo": "dezacord total",
    "scale_hi": "acord total",
    "pt_budget_each": (n)=>`${n} puncte de persoană`,
    "live_open": "DESCHIS",
    "live_closed_b": "ÎNCHIS",
    "live_need_prompt": "Scrie o întrebare sau o temă.",
    "live_need_opts": "Adaugă cel puțin 2 opțiuni.",
    "live_need_stmts": "Adaugă cel puțin 2 afirmații.",
    "sv_need_q": "Scrie măcar o întrebare.",
    "live_ended": "Sesiune închisă.",
    "live_copied": "Link copiat!",
    "live_json_name": "rezultate-live",
    "live_png_name": "nor-de-cuvinte",
    "live_back": "Închide",
    "live_mode_survey": "Chestionar",
    "live_pause": "Pauză",
    "live_resume": "Reia",
    "live_clear": "Golește",
    "live_end": "Termină",
    "live_join_at": "Intră la",
    "live_copy": "Copiază linkul",
    "deck_prev": "Înapoi",
    "deck_slide": "Activitatea",
    "deck_next": "Următoarea",
    "live_share": "Distribuie",
    "live_need_code": "Introdu un cod.",
    "live_notfound": "Sesiunea nu există sau s-a încheiat.",
    "live_closed": "Răspunsurile sunt închise.",
    "live_need_text": "Scrie ceva mai întâi.",
    "live_sent_mod": "Trimisă spre aprobare.",
    "live_sent": "Trimis!",
    "live_slow": "Prea repede — așteaptă o clipă.",
    "scale_all": "Notează toate afirmațiile.",
    "pt_useall": "Folosește toate punctele",
    "sv_need_ans": "Scrie măcar un răspuns.",
    "sv_thanks": "Mulțumim! Răspunsul tău a fost trimis.",
    "live_leave": "Ieși",
    "sv_word_ph": "Un cuvânt sau o frază scurtă…",
    "live_closed_note": "Gazda a pus răspunsurile pe pauză.",
    "sv_send": "Trimite",
    "live_loading": "Se încarcă…",
    "live_voted": "Vot înregistrat!",
    "live_qa_ph": "Scrie întrebarea sau ideea ta…",
    "live_send": "Trimite",
    "nps_low": "Deloc probabil",
    "nps_high": "Extrem de probabil",
    "rank_submit": "Trimite ordinea",
    "pt_remaining": "Rămase:",
    "live_word_ph": "Scrie un cuvânt…",
    "live_words_left": (n)=>`Mai poți trimite ${n} cuvânt(e).`,
    "live_thanks_words": "Mulțumim! Vezi norul de cuvinte mai jos.",
    "live_live_results": "Rezultate live",
    "live_options": "Opțiuni",
    "live_option": "Opțiunea",
    "live_add_option": "Adaugă opțiune",
    "live_rank_opts": "Opțiuni de ordonat",
    "live_rating_scale": "Tip de scală",
    "live_statements": "Afirmații",
    "live_statement": "Afirmația",
    "live_add_stmt": "Adaugă afirmație",
    "live_scale_hint": "Fiecare afirmație se notează pe o scală de la 1 (dezacord total) la 5 (acord total).",
    "live_points_opts": "Opțiuni",
    "live_points_hint": "Fiecare participant împarte 100 de puncte între aceste opțiuni.",
    "live_qa_mod": "Moderare întrebări",
    "live_qa_mod_off": "Fără (toate apar)",
    "live_qa_mod_on": "Cu aprobare",
    "live_qa_mod_hint": "Cu aprobare, întrebările apar publicului doar după ce le aprobi tu.",
    "live_filter": "Filtru de limbaj",
    "live_filter_off": "Oprit",
    "live_filter_on": "Pornit",
    "live_filter_hint": "Ascunde automat cuvintele vulgare din răspunsuri (le înlocuiește cu ***).",
    "live_max_words": "Câte cuvinte poate trimite o persoană",
    "live_mode_single": "O activitate",
    "live_mode_deck": "Prezentare",
    "live_host_login": "Autentifică-te ca administrator pentru a găzdui o sesiune.",
    "deck_empty": "Nicio activitate încă. Alege un tip mai sus și adaugă.",
    "deck_title": "Titlul prezentării",
    "deck_title_ph": "ex. Atelier Atlantykron — feedback",
    "deck_slides": "Activități",
    "deck_add_new": "Adaugă o activitate",
    "live_prompt": "Întrebarea / tema",
    ...
    "sv_title": "Titlul chestionarului",
    "sv_title_ph": "Ex: Feedback eveniment ASFAN",
    ...
    "live_join_code": "Intră cu un cod",
    "live_code_ph": "COD",
    "live_join": "Intră",
    "live_host_new": "Sau pornește o activitate nouă",
    "deck_imported": (n)=>`${n} activități importate.`,
    "deck_full": "Maximum 50 de activități.",
    "deck_added": "Activitate adăugată.",
    "deck_need_slides": "Adaugă cel puțin o activitate.",
    "deck_default_title": "Prezentare",
    "deck_import_bad": "Fișier invalid.",
    "deck_import_empty": "Fișierul nu conține activități valide.",
    "game_waiting_players": "Aștept jucători… împărtășește codul!",
    "game_players": "jucători",
    "game_start": "Începe jocul",
    "gh_typing": "Participanții își scriu răspunsul…",
    "game_reveal": "Arată răspunsul",
    "gh_gotit": "corecte",
    "game_results": "Rezultate",
    "game_next": "Următoarea",
    "team_standings": "Clasament pe echipe",
    "game_replay": "Joacă din nou",
    "game_report": "Raport",
    "game_need_name": "Scrie o poreclă.",
    "game_lobby_closed": "Jocul a început deja.",
    "game_times_up": "A expirat timpul!",
    "game_pick_name": "Alege un nume și un avatar",
    "game_nickname": "Porecla ta",
    "nick_gen": "Poreclă la întâmplare",
    "team_pick": "Alege-ți echipa",
    "game_enter": "Intră în joc",
    "game_youre_in": "Ești în joc!",
    "game_look_screen": "Privește ecranul principal.",
    "game_locked": "Răspuns trimis!",
    "game_wait_others": "Așteaptă ceilalți jucători…",
    "game_correct": "Corect!",
    "game_wrong": "Greșit",
    "game_missed": "Fără răspuns",
    "game_your_rank": "Locul tău:",
    "game_finished": "Joc terminat!",
    "report_back": "Înapoi",
    "report_no_data": "Încă nu sunt date.",
    "report_players": "jucători",
    "report_questions": "întrebări",
    "report_avg_score": "scor mediu",
    "report_accuracy": "acuratețe",
    "report_avg_time": "timp mediu",
    "report_easy": "ușoară",
    "report_med": "medie",
    "report_hard": "grea",
    "report_correct": "corecte",
    "report_answered": "au răspuns",
    "report_player": "Jucător",
    "report_csv": "CSV",
    "report_json": "JSON",
    "report_print": "Printează",
    "report_title": "Raport joc",
    "report_byq": "Pe întrebări",
    "report_byp": "Pe jucători",
    "assign_none_done": "Încă nimeni n-a terminat.",
    "assign_done": "au terminat",
    "assign_joined": "înscriși",
    "assign_closed": "Tema este închisă.",
    "assign_closed_note": "Gazda a închis tema.",
    "assign_intro": (n)=>`${n} întrebări · în ritmul tău`,
    "assign_start": "Începe",
    "assign_finish": "Vezi rezultatul",
    "assign_complete": "Ai terminat tema!",
    "offline_feature": "Indisponibil offline.",
    "offline_banner": "Ești offline — solo, biblioteca și editorul funcționează. Modurile live revin la reconectare.",
    "install_hint": "Instalează Undava ca aplicație",
    "install_btn": "Instalează",
    "online_back": "Ești din nou online.",
    "live_clear_q": "Ștergi toate răspunsurile?",
    "live_end_q": "Închizi definitiv sesiunea? Datele se șterg."
  },
  "en": {
    "print_quiz": "Print",
    "unsaved_q": "You have unsaved changes. Leave without saving?",
    "review_btn": "Review",
    "review_title": "Answer review",
    "review_answer": "Correct:",
    "review_back": "Back to score",
    "sort_by": "Sort",
    "sort_default": "Default",
    "sort_name": "Name A-Z",
    "sort_questions": "Question count",
    "dup_q": "Duplicate question",
    "lib_search": "Search quizzes…",
    "lib_nomatch": "No quizzes found.",
    "tagline": "Quiz without borders",
    "set_sound": "Sound",
    "settings": "Settings",
    "spin_need": "Add at least 2 items.",
    "back_home": "Home",
    "spin_title": "Spinner wheel",
    "spin_sub": "Enter names or options, then spin to pick at random.",
    "spin_go": "Spin",
    "spin_items": "Items",
    "spin_ph": "One item per line…\nAlice\nBob\nCarol",
    "spin_elim": "Remove the winner after each spin",
    "spin_shuffle": "Shuffle",
    "spin_reset": "Reset",
    "spin_default": "Alice|Bob|Carol|Dave|Eve|Frank",
    "help_book_eli5": "Quick guide",
    "help_book_user": "User manual",
    "help_book_admin": "Admin manual",
    "help_title": "User guide",
    "help_sub": "Complete step-by-step manual.",
    ...
    "feat_offline": "100% offline",
    "feat_free": "Trade-free",
    "feat_priv": "Zero telemetry",
    "feat_open": "Single file",
    "tf_desc": "no ads, no data collection, no account needed, no features locked behind payment",
    "tf_tools": "other tools",
    ...
    "export": "Export",
    "del": "Delete",
    "lib_title": "Quiz library",
    "lib_sub": "Pick a quiz to play or edit",
    "lib_mine": "My quizzes",
    "export_all": "Export all",
    "lib_empty": "You haven't created any quizzes yet. Tap “Create quiz” to start.",
    "lib_samples": "Sample quizzes",
    "library": "My quizzes",
    "new_quiz": "New quiz",
    "edit_quiz": "Edit quiz",
    "quiz_title": "Quiz title",
    "quiz_title_ph": "e.g. General knowledge",
    "quiz_desc": "Description (optional)",
    "quiz_desc_ph": "A short description…",
    "questions": "Questions",
    "add_question": "Add question",
    "cancel": "Cancel",
    "save": "Save quiz",
    "acc_primary_ph": "The correct answer (shown)",
    "acc_alt_ph": "Accepted variant (synonym, spelling)",
    "acc_add": "Add accepted variant",
    "acc_hint": "Small typos are accepted too. The first variant is shown as the correct answer.",
    "num_target": "Correct number",
    "num_target_ph": "e.g. 42",
    "num_tol": "Accepted tolerance",
    ...
    "question_n": (n)=>"Question "+n,
    "type_quiz": "Multiple choice (4)",
    "type_tf": "True / False",
    "type_type": "Free answer",
    "type_num": "Numeric (guess)",
    "sec": "s",
    "pts_std": "Standard",
    "pts_dbl": "Double",
    "question_ph": "Type the question here…",
    "choose_mode": "Game mode",
    "mode_solo": "Solo",
    "mode_solo_d": "Play alone and beat your high score.",
    "mode_hot": "Pass & play",
    "mode_hot_d": "Several players on the same device.",
    "shuffle_opt": "Shuffle questions & answers",
    "players": "Players",
    ...
    "import_title": "Import a quiz",
    "import_drop": "Drop a .json file here or tap to choose",
    "import_paste": "…or paste JSON content:",
    "import_paste_ph": "{\"title\":\"…\",\"questions\":[…]}",
    "import_btn": "Import",
    "import_fmt_title": "JSON format — create manually",
    "import_fmt_intro": "You can write a .json file with the structure below and import it here. Here is an example with all question types:",
    "import_copy": "Copy example",
    "import_template": "Download template",
    "import_fields": "FILE STRUCTURE\n• title — the quiz name\n• desc — short description (optional)\n• color — hex color, e.g. #2f6bff (optional)\n• questions — the list of questions\n\nEACH QUESTION\n• type — quiz (choices) / tf (true-false) / type (typed answer) / num (numeric)\n• text — the question\n• time — seconds: 5, 10, 20, 30, 60 or 120\n• points — 1000 (standard) or 2000 (double)\n• answers — the answers (see below)\n• tol — num only: accepted tolerance (±)\n• explain — optional explanation (why it's correct)\n\nANSWERS, BY TYPE\n• quiz: 2–4 choices; mark the correct one with \"correct\": true\n• tf: 2 choices (True / False); mark the correct one\n• type: 1–6 accepted texts (all correct; add spelling variants)\n• num: a single number; optional tol for tolerance",
    "import_ok": "Quiz imported!",
    "import_err": "Invalid file. Check the JSON format.",
    "you": "You",
    "need_player": "Add at least one player.",
    "scoreboard": "Scoreboard",
    "q_of": (a,b)=>"Question "+a+" of "+b,
    "pass_to": "Pass the device to",
    "tap_start": "Tap when you're ready",
    "ready_q": "Ready?",
    "num_answer_ph": "Type a number…",
    "type_answer_ph": "Type your answer…",
    "type_submit": "Submit",
    "keys_hint": "Press 1-4 or tap an answer",
    "verdict_ok": "Correct!",
    "verdict_no": "Wrong!",
    "verdict_miss": "No answer",
    ...
    "podium_title": "Congratulations!",
    "winner_is": "Winner",
    "type_need_answer": "Type an answer.",
    "set_title": "Settings",
    "set_lang": "Language",
    "set_lang_d": "Interface language",
    "set_sound_d": "Sound effects during play",
    ...
    "backup_done": "Backup saved",
    "quit_q": "Leave the game? Progress will be lost.",
    "err_title": "Give the quiz a title.",
    "err_noq": "Add at least one question.",
    "err_qtext": (n)=>"Question "+n+" has no text.",
    "err_acc": (n)=>`Question ${n}: add at least one accepted answer.`,
    "err_num": (n)=>`Question ${n}: enter a valid number.`,
    "err_ans": (n)=>"Question "+n+" needs at least 2 answers.",
    "err_corr": (n)=>"Mark the correct answer for question "+n+".",
    "explain_ph": "Explanation (optional) — why this is correct",
    "net_err": "Network error. Check your connection.",
    "live_t_cloud": "Word cloud",
    "live_t_cloud_d": "The audience sends words, shown live.",
    "live_t_poll": "Live poll",
    "live_t_poll_d": "Vote on options, real-time bars.",
    "live_t_qa": "Questions & ideas",
    "live_t_qa_d": "Upvotable messages from the crowd.",
    "live_t_rating": "Rating / NPS",
    "live_t_rating_d": "Star rating or NPS 0–10 score.",
    "live_t_rank": "Ranking",
    "live_t_rank_d": "Participants order the options by preference.",
    "live_t_scale": "Scales (Likert)",
    "live_t_scale_d": "Several statements rated 1–5 (disagree→agree).",
    "live_t_points": "100 points",
    "live_t_points_d": "Participants split a points budget across options.",
    "cat_quiz": "Quizzes & games",
    "cat_quiz_d": "Multiple choice, true/false, free answer, numeric · Solo, turns, live, homework or teams",
    "cat_audience": "Audience activities",
    "cat_audience_d": "Collect live responses from the audience, from their phones. Pick a type:",
    "cat_deck": "Presentations",
    "cat_deck_d": "Chain several activities into one session, navigated at your pace.",
    "deck_build": "Build a presentation",
    "cat_survey": "Feedback survey",
    "cat_tools": "Tools",
    "live_nav": "Live",
    "guestbook": "Feedback",
    "mode_live": "Live (multiplayer)",
    "mode_live_d": "Host on the big screen; the audience plays on phones.",
    "mode_assign": "Self-paced",
    "mode_assign_d": "Assign a quiz; everyone solves it on their own, at their own pace.",
    "team_opt": "Team mode",
    "team_word": "teams",
    "assign_note": "Participants join from their phones and solve at their own pace. Results collect here.",
    "game_join_note": "Players join from their phones by scanning the QR code or entering the displayed code.",
    "game_host": "Host live",
    "assign_host": "Assign homework",
    "fb_leave": "Leave feedback",
    "set_admin": "Administrator",
    "set_admin_on": "You are signed in as administrator.",
    "set_admin_off": "Sign in to edit quizzes.",
    "logout": "Sign out",
    "login": "Sign in",
    "password": "Password",
    "password_again": "Repeat password",
    "fb_need_rating": "Pick a rating (1–5 stars).",
    "fb_need_msg": "Write a message.",
    "fb_thanks_mod": "Thanks! Your message will appear after approval.",
    ...
    "admin_out": "Signed out.",
    "fb_empty": "No feedback yet. Be the first!",
    "fb_title": "Visitor feedback",
    "fb_sub": "Leave a thought about the quizzes and see what others wrote.",
    "fb_recent": "Recent feedback",
    "fb_local_note": "(saved locally, on this device)",
    "fb_pick_quiz": "— pick a quiz —",
    "fb_name": "Name (optional)",
    "fb_name_ph": "What should we call you?",
    "fb_quiz": "Quiz (optional)",
    "fb_rating": "Rating",
    "fb_msg": "Message",
    "fb_msg_ph": "What did you like? What could be better?",
    ...
    "qa_approve": "Approve",
    "qa_star": "Highlight",
    "qa_mark": "Mark as answered",
    "live_show": "Show",
    "live_hide": "Hide",
    "qa_pending_count": (n)=>`${n} pending`,
    "live_waiting": "Waiting for responses…",
    "live_responses": "responses",
    "rt_prom": "Promoters",
    "rt_pass": "Passives",
    "rt_det": "Detractors",
    "rt_avg": "average",
    "rk_foot": "ordered by average rank (lower = better)",
    "scale_lo": "strongly disagree",
    "scale_hi": "strongly agree",
    "pt_budget_each": (n)=>`${n} points each`,
    "live_open": "OPEN",
    "live_closed_b": "CLOSED",
    "live_need_prompt": "Write a question or topic.",
    "live_need_opts": "Add at least 2 options.",
    "live_need_stmts": "Add at least 2 statements.",
    "sv_need_q": "Write at least one question.",
    "live_ended": "Session closed.",
    "live_copied": "Link copied!",
    "live_json_name": "live-results",
    "live_png_name": "word-cloud",
    "live_back": "Close",
    "live_mode_survey": "Survey",
    "live_pause": "Pause",
    "live_resume": "Resume",
    "live_clear": "Clear",
    "live_end": "End",
    "live_join_at": "Join at",
    "live_copy": "Copy link",
    "deck_prev": "Previous",
    "deck_slide": "Activity",
    "deck_next": "Next",
    "live_share": "Share",
    "live_need_code": "Enter a code.",
    "live_notfound": "Session not found or has ended.",
    "live_closed": "Responses are closed.",
    "live_need_text": "Write something first.",
    "live_sent_mod": "Sent for approval.",
    "live_sent": "Sent!",
    "live_slow": "Too fast — wait a moment.",
    "scale_all": "Rate every statement.",
    "pt_useall": "Use all your points",
    "sv_need_ans": "Write at least one answer.",
    "sv_thanks": "Thank you! Your response was sent.",
    "live_leave": "Leave",
    "sv_word_ph": "A word or a short phrase…",
    "live_closed_note": "The host paused responses.",
    "sv_send": "Send",
    "live_loading": "Loading…",
    "live_voted": "Vote counted!",
    "live_qa_ph": "Type your question or idea…",
    "live_send": "Send",
    "nps_low": "Not at all likely",
    "nps_high": "Extremely likely",
    "rank_submit": "Submit ranking",
    "pt_remaining": "Remaining:",
    "live_word_ph": "Type a word…",
    "live_words_left": (n)=>`You can send ${n} more word(s).`,
    "live_thanks_words": "Thanks! See the word cloud below.",
    "live_live_results": "Live results",
    "live_options": "Options",
    "live_option": "Option",
    "live_add_option": "Add option",
    "live_rank_opts": "Options to rank",
    "live_rating_scale": "Scale type",
    "live_statements": "Statements",
    "live_statement": "Statement",
    "live_add_stmt": "Add statement",
    "live_scale_hint": "Each statement is rated on a 1 (strongly disagree) to 5 (strongly agree) scale.",
    "live_points_opts": "Options",
    "live_points_hint": "Each participant splits 100 points across these options.",
    "live_qa_mod": "Question moderation",
    "live_qa_mod_off": "Off (all shown)",
    "live_qa_mod_on": "Require approval",
    "live_qa_mod_hint": "With approval, questions appear to the audience only after you approve them.",
    "live_filter": "Profanity filter",
    "live_filter_off": "Off",
    "live_filter_on": "On",
    "live_filter_hint": "Automatically masks vulgar words in responses (replaces them with ***).",
    "live_max_words": "How many words one person can send",
    "live_mode_single": "Single activity",
    "live_mode_deck": "Presentation",
    "live_host_login": "Sign in as administrator to host a session.",
    "deck_empty": "No activities yet. Pick a type above and add one.",
    "deck_title": "Presentation title",
    "deck_title_ph": "e.g. Atlantykron workshop — feedback",
    "deck_slides": "Activities",
    "deck_add_new": "Add an activity",
    "live_prompt": "Question / topic",
    ...
    "sv_title": "Survey title",
    "sv_title_ph": "E.g. ASFAN event feedback",
    ...
    "live_join_code": "Join with a code",
    "live_code_ph": "CODE",
    "live_join": "Join",
    "live_host_new": "Or start a new activity",
    "deck_imported": (n)=>`${n} activities imported.`,
    "deck_full": "Maximum 50 activities.",
    "deck_added": "Activity added.",
    "deck_need_slides": "Add at least one activity.",
    "deck_default_title": "Presentation",
    "deck_import_bad": "Invalid file.",
    "deck_import_empty": "No valid activities in the file.",
    "game_waiting_players": "Waiting for players… share the code!",
    "game_players": "players",
    "game_start": "Start game",
    "gh_typing": "Participants are typing…",
    ...
    "game_report": "Report",
    "game_need_name": "Enter a nickname.",
    "game_lobby_closed": "The game already started.",
    "game_times_up": "Time's up!",
    "game_pick_name": "Pick a name and avatar",
    "game_nickname": "Your nickname",
    "nick_gen": "Random nickname",
    "team_pick": "Pick your team",
    "game_enter": "Enter game",
    "game_youre_in": "You're in!",
    "game_look_screen": "Look at the main screen.",
    "game_locked": "Answer submitted!",
    "game_wait_others": "Waiting for other players…",
    "game_correct": "Correct!",
    "game_wrong": "Wrong",
    "game_missed": "No answer",
    "game_your_rank": "Your rank:",
    ...
    "report_byq": "By question",
    "report_byp": "By player",
    "assign_none_done": "No one has finished yet.",
    "assign_done": "finished",
    "assign_joined": "joined",
    "assign_closed": "This assignment is closed.",
    "assign_closed_note": "The host closed the assignment.",
    "assign_intro": (n)=>`${n} questions · at your pace`,
    "assign_start": "Start",
    "assign_finish": "See result",
    "assign_complete": "You finished!",
    "offline_feature": "Unavailable offline.",
    "offline_banner": "You're offline — solo, library and editor work. Live modes return when reconnected.",
    "install_hint": "Install Undava as an app",
    "install_btn": "Install",
    "online_back": "You're back online.",
    "live_clear_q": "Delete all responses?",
    "live_end_q": "End the session for good? Data will be deleted."
  },
  "fr": {
    "print_quiz": "Imprimer",
    "unsaved_q": "Tu as des modifications non enregistrées. Quitter sans enregistrer ?",
    "review_btn": "Récap",
    "review_title": "Récapitulatif",
    "review_answer": "Correct :",
    "review_back": "Retour au score",
    "sort_by": "Trier",
    "sort_default": "Défaut",
    "sort_name": "Nom A-Z",
    "sort_questions": "Nb de questions",
    "dup_q": "Dupliquer la question",
    "lib_search": "Rechercher un quiz…",
    "lib_nomatch": "Aucun quiz trouvé.",
    "tagline": "Quiz sans frontières",
    "set_sound": "Son",
    "settings": "Paramètres",
    "spin_need": "Ajoute au moins 2 éléments.",
    "back_home": "Accueil",
    "spin_title": "Roue de la fortune",
    "spin_sub": "Saisis des noms ou des options, puis fais tourner pour choisir au hasard.",
    "spin_go": "Tourner",
    "spin_items": "Éléments",
    "spin_ph": "Un élément par ligne…\nAlice\nBob\nCarole",
    "spin_elim": "Retirer le gagnant après chaque tirage",
    "spin_shuffle": "Mélanger",
    "spin_reset": "Réinitialiser",
    "spin_default": "Alice|Bob|Carole|David|Éva|Franck",
    "help_book_eli5": "Guide express",
    "help_book_user": "Manuel utilisateur",
    "help_book_admin": "Manuel administrateur",
    "help_title": "Guide d'utilisation",
    "help_sub": "Manuel complet, étape par étape.",
    ...
    "spin_nav": "Roue",
    "help_nav": "Guide",
    "feat_offline": "100% hors ligne",
    "feat_free": "Sans commerce",
    "feat_priv": "Zéro télémétrie",
    "feat_open": "Fichier unique",
    "tf_desc": "sans publicité, sans collecte de données, sans compte requis, sans fonctions bloquées derrière un paiement",
    "tf_tools": "autres outils",
    "q_count": (n)=>n+(n===1?" question":" questions"),
    "playbtn": "Jouer",
    "edit": "Modifier",
    "duplicate": "Dupliquer",
    "export": "Exporter",
    "del": "Supprimer",
    "lib_title": "Bibliothèque de quiz",
    "lib_sub": "Choisis un quiz à jouer ou à modifier",
    "lib_mine": "Mes quiz",
    "export_all": "Tout exporter",
    "lib_empty": "Tu n'as encore créé aucun quiz. Appuie sur « Créer un quiz » pour commencer.",
    "lib_samples": "Quiz d'exemple",
    "library": "Mes quiz",
    "new_quiz": "Nouveau quiz",
    "edit_quiz": "Modifier le quiz",
    "quiz_title": "Titre du quiz",
    "quiz_title_ph": "ex. Culture générale",
    "quiz_desc": "Description (facultatif)",
    "quiz_desc_ph": "Une courte description…",
    "questions": "Questions",
    "add_question": "Ajouter une question",
    "cancel": "Annuler",
    "save": "Enregistrer le quiz",
    "acc_primary_ph": "La réponse correcte (affichée)",
    "acc_alt_ph": "Variante acceptée (synonyme, orthographe)",
    "acc_add": "Ajouter une variante acceptée",
    "acc_hint": "Les petites fautes de frappe sont acceptées. La première variante est affichée comme réponse correcte.",
    "num_target": "Nombre correct",
    "num_target_ph": "ex. 42",
    "num_tol": "Tolérance acceptée",
    ...
    "question_n": (n)=>"Question "+n,
    "type_quiz": "Choix multiple (4)",
    "type_tf": "Vrai / Faux",
    "type_type": "Réponse libre",
    "type_num": "Numérique (devine)",
    "sec": "s",
    "pts_std": "Standard",
    "pts_dbl": "Double",
    "question_ph": "Écris la question ici…",
    "choose_mode": "Mode de jeu",
    "mode_solo": "Solo",
    "mode_solo_d": "Joue seul et bats ton record.",
    "mode_hot": "Chacun son tour",
    "mode_hot_d": "Plusieurs joueurs sur le même appareil.",
    "shuffle_opt": "Mélanger questions et réponses",
    "players": "Joueurs",
    ...
    "import_copied": "Copié !",
    "import_title": "Importer un quiz",
    "import_drop": "Dépose un fichier .json ici ou touche pour choisir",
    "import_paste": "…ou colle le contenu JSON :",
    "import_paste_ph": "{\"title\":\"…\",\"questions\":[…]}",
    "import_btn": "Importer",
    "import_fmt_title": "Format JSON — créer à la main",
    "import_fmt_intro": "Tu peux écrire un fichier .json avec la structure ci-dessous et l'importer ici. Voici un exemple avec tous les types de questions :",
    "import_copy": "Copier l'exemple",
    "import_template": "Télécharger le modèle",
    "import_fields": "STRUCTURE DU FICHIER\n• title — le nom du quiz\n• desc — brève description (facultatif)\n• color — couleur hex, par ex. #2f6bff (facultatif)\n• questions — la liste des questions\n\nCHAQUE QUESTION\n• type — quiz (choix) / tf (vrai-faux) / type (réponse écrite) / num (numérique)\n• text — l'énoncé\n• time — secondes : 5, 10, 20, 30, 60 ou 120\n• points — 1000 (standard) ou 2000 (double)\n• answers — les réponses (voir ci-dessous)\n• tol — num uniquement : tolérance acceptée (±)\n• explain — explication facultative (pourquoi c'est correct)\n\nRÉPONSES, PAR TYPE\n• quiz : 2–4 choix ; marque le bon avec \"correct\": true\n• tf : 2 choix (Vrai / Faux) ; marque le bon\n• type : 1–6 textes acceptés (tous corrects ; ajoute des variantes)\n• num : un seul nombre ; tol facultatif pour la tolérance",
    "import_ok": "Quiz importé !",
    "import_err": "Fichier invalide. Vérifie le format JSON.",
    "you": "Toi",
    "need_player": "Ajoute au moins un joueur.",
    "scoreboard": "Classement",
    "q_of": (a,b)=>"Question "+a+" sur "+b,
    "pass_to": "Passe l'appareil à",
    "tap_start": "Touche quand tu es prêt",
    "ready_q": "Prêt ?",
    "num_answer_ph": "Écris un nombre…",
    "type_answer_ph": "Écris ta réponse…",
    "type_submit": "Envoyer",
    "keys_hint": "Appuie sur 1-4 ou touche une réponse",
    "verdict_ok": "Correct !",
    "verdict_no": "Faux !",
    "verdict_miss": "Pas de réponse",
    "correct_was": "Bonne réponse :",
    "you_wrote": "Tu as écrit :",
    "streak": (n)=>n+" d'affilée 🔥",
    "continue": "Continuer",
    "fastest5": "Les plus rapides",
    "stat_pts": "Points",
    "stat_correct": "Correctes",
    "stat_acc": "Précision",
    "stat_streak": "Meilleure série",
    "play_again": "Rejouer",
    "podium_title": "Félicitations !",
    "winner_is": "Gagnant",
    "type_need_answer": "Écris une réponse.",
    "set_title": "Paramètres",
    "set_lang": "Langue",
    "set_lang_d": "Langue de l'interface",
    "set_sound_d": "Effets sonores pendant le jeu",
    ...
    "close": "Fermer",
    "saved": "Quiz enregistré !",
    "confirm_del": "Supprimer ce quiz définitivement ?",
    "deleted": "Quiz supprimé",
    "backup_done": "Sauvegarde enregistrée",
    "quit_q": "Quitter la partie ? La progression sera perdue.",
    "err_title": "Donne un titre au quiz.",
    "err_noq": "Ajoute au moins une question.",
    "err_qtext": (n)=>"La question "+n+" n'a pas de texte.",
    "err_acc": (n)=>`Question ${n} : ajoute au moins une réponse acceptée.`,
    "err_num": (n)=>`Question ${n} : saisis un nombre valide.`,
    "err_ans": (n)=>"La question "+n+" a besoin d'au moins 2 réponses.",
    "err_corr": (n)=>"Indique la bonne réponse pour la question "+n+".",
    "explain_ph": "Explication (facultatif) — pourquoi c'est correct",
    "net_err": "Erreur réseau. Vérifie ta connexion.",
    "live_t_cloud": "Nuage de mots",
    "live_t_cloud_d": "Le public envoie des mots, affichés en direct.",
    "live_t_poll": "Sondage en direct",
    "live_t_poll_d": "Vote sur des options, barres en temps réel.",
    "live_t_qa": "Questions et idées",
    "live_t_qa_d": "Messages du public, avec votes.",
    "live_t_rating": "Note / NPS",
    "live_t_rating_d": "Note en étoiles ou score NPS 0–10.",
    "live_t_rank": "Classement",
    "live_t_rank_d": "Les participants ordonnent les options par préférence.",
    "live_t_scale": "Échelles (Likert)",
    "live_t_scale_d": "Plusieurs affirmations notées de 1 à 5 (pas d'accord→d'accord).",
    "live_t_points": "100 points",
    "live_t_points_d": "Les participants répartissent un budget de points entre les options.",
    "cat_quiz": "Quiz & jeux",
    "cat_quiz_d": "Choix multiple, vrai/faux, réponse libre, numérique · Solo, à tour de rôle, live, devoir ou équipes",
    "cat_audience": "Activités d'audience",
    "cat_audience_d": "Collecte les réponses du public en direct, depuis les téléphones. Choisis un type :",
    "cat_deck": "Présentations",
    "cat_deck_d": "Enchaîne plusieurs activités en une seule session, à ton rythme.",
    "deck_build": "Créer une présentation",
    "cat_survey": "Sondage de retour",
    "cat_tools": "Outils",
    "live_nav": "En direct",
    "guestbook": "Commentaires",
    "mode_live": "En direct (multijoueur)",
    "mode_live_d": "Anime sur le grand écran ; le public joue sur téléphone.",
    "mode_assign": "À son rythme",
    "mode_assign_d": "Attribue un quiz ; chacun le résout seul, à son rythme.",
    "team_opt": "Mode équipes",
    "team_word": "équipes",
    "assign_note": "Les participants rejoignent depuis leur téléphone et résolvent à leur rythme. Les résultats s'accumulent ici.",
    "game_join_note": "Les joueurs rejoignent depuis leur téléphone en scannant le QR code ou en saisissant le code affiché.",
    "game_host": "Animer en direct",
    "assign_host": "Attribuer un devoir",
    "fb_leave": "Laisser un commentaire",
    "set_admin": "Administrateur",
    "set_admin_on": "Tu es connecté en tant qu'administrateur.",
    "set_admin_off": "Connecte-toi pour modifier les quiz.",
    "logout": "Se déconnecter",
    "login": "Se connecter",
    "password": "Mot de passe",
    "password_again": "Répète le mot de passe",
    "fb_need_rating": "Choisis une note (1 à 5 étoiles).",
    "fb_need_msg": "Écris un message.",
    "fb_thanks_mod": "Merci ! Ton message apparaîtra après approbation.",
    "fb_thanks": "Merci pour ton commentaire !",
    "err_pw": "Saisis le mot de passe.",
    "admin_ok": "Connecté !",
    "admin_bad": "Mot de passe incorrect.",
    "err_pw_short": "Le mot de passe doit comporter au moins 6 caractères.",
    "err_pw_match": "Les mots de passe ne correspondent pas.",
    "admin_out": "Déconnecté.",
    "fb_empty": "Aucun commentaire pour l'instant. Sois le premier !",
    "fb_title": "Commentaires des visiteurs",
    "fb_sub": "Laisse un mot sur les quiz et vois ce que les autres ont écrit.",
    "fb_recent": "Commentaires récents",
    "fb_local_note": "(enregistré localement, sur cet appareil)",
    "fb_pick_quiz": "— choisis un quiz —",
    "fb_name": "Nom (facultatif)",
    "fb_name_ph": "Comment t'appeler ?",
    "fb_quiz": "Quiz (facultatif)",
    "fb_rating": "Note",
    "fb_msg": "Message",
    "fb_msg_ph": "Qu'as-tu aimé ? Que pourrait-on améliorer ?",
    "fb_send": "Envoyer",
    "fb_anon": "Anonyme",
    "fb_pending": "en attente",
    "fb_approve": "Approuver",
    "qa_pending": "En attente",
    "qa_answered": "Répondu",
    "qa_approve": "Approuver",
    "qa_star": "Mettre en avant",
    "qa_mark": "Marquer comme répondu",
    "live_show": "Afficher",
    "live_hide": "Masquer",
    "qa_pending_count": (n)=>`${n} en attente`,
    "live_waiting": "En attente de réponses…",
    "live_responses": "réponses",
    "rt_prom": "Promoteurs",
    "rt_pass": "Passifs",
    "rt_det": "Détracteurs",
    "rt_avg": "moyenne",
    "rk_foot": "classé par rang moyen (plus bas = meilleur)",
    "scale_lo": "pas du tout d'accord",
    "scale_hi": "tout à fait d'accord",
    "pt_budget_each": (n)=>`${n} points chacun`,
    "live_open": "OUVERT",
    "live_closed_b": "FERMÉ",
    "live_need_prompt": "Écris une question ou un sujet.",
    "live_need_opts": "Ajoute au moins 2 options.",
    "live_need_stmts": "Ajoute au moins 2 affirmations.",
    "sv_need_q": "Écris au moins une question.",
    "live_ended": "Session fermée.",
    "live_copied": "Lien copié !",
    "live_json_name": "resultats-live",
    "live_png_name": "nuage-de-mots",
    "live_back": "Fermer",
    "live_mode_survey": "Sondage",
    "live_pause": "Pause",
    "live_resume": "Reprendre",
    "live_clear": "Effacer",
    "live_end": "Terminer",
    "live_join_at": "Rejoins sur",
    "live_copy": "Copier le lien",
    "deck_prev": "Précédent",
    "deck_slide": "Activité",
    "deck_next": "Suivant",
    "live_share": "Partager",
    "live_need_code": "Saisis un code.",
    "live_notfound": "Session introuvable ou terminée.",
    "live_closed": "Les réponses sont closes.",
    "live_need_text": "Écris quelque chose d'abord.",
    "live_sent_mod": "Envoyé pour approbation.",
    "live_sent": "Envoyé !",
    "live_slow": "Trop vite — attends un instant.",
    "scale_all": "Note chaque affirmation.",
    "pt_useall": "Utilise tous tes points",
    "sv_need_ans": "Écris au moins une réponse.",
    "sv_thanks": "Merci ! Ta réponse a été envoyée.",
    "live_leave": "Quitter",
    "sv_word_ph": "Un mot ou une courte phrase…",
    "live_closed_note": "L'animateur a suspendu les réponses.",
    "sv_send": "Envoyer",
    "live_loading": "Chargement…",
    "live_voted": "Vote compté !",
    "live_qa_ph": "Écris ta question ou ton idée…",
    "live_send": "Envoyer",
    "nps_low": "Pas du tout probable",
    "nps_high": "Extrêmement probable",
    "rank_submit": "Envoyer le classement",
    "pt_remaining": "Restant :",
    "live_word_ph": "Écris un mot…",
    "live_words_left": (n)=>`Tu peux encore envoyer ${n} mot(s).`,
    "live_thanks_words": "Merci ! Vois le nuage de mots ci-dessous.",
    "live_live_results": "Résultats en direct",
    "live_options": "Options",
    "live_option": "Option",
    "live_add_option": "Ajouter une option",
    "live_rank_opts": "Options à classer",
    "live_rating_scale": "Type d'échelle",
    "live_statements": "Affirmations",
    "live_statement": "Affirmation",
    "live_add_stmt": "Ajouter une affirmation",
    "live_scale_hint": "Chaque affirmation est notée sur une échelle de 1 (pas du tout d'accord) à 5 (tout à fait d'accord).",
    "live_points_opts": "Options",
    "live_points_hint": "Chaque participant répartit 100 points entre ces options.",
    "live_qa_mod": "Modération des questions",
    "live_qa_mod_off": "Désactivée (tout s'affiche)",
    "live_qa_mod_on": "Exiger une approbation",
    "live_qa_mod_hint": "Avec approbation, les questions n'apparaissent au public qu'après ton approbation.",
    "live_filter": "Filtre anti-grossièretés",
    "live_filter_off": "Désactivé",
    "live_filter_on": "Activé",
    "live_filter_hint": "Masque automatiquement les mots vulgaires dans les réponses (les remplace par ***).",
    "live_max_words": "Combien de mots une personne peut envoyer",
    "live_mode_single": "Activité unique",
    "live_mode_deck": "Présentation",
    "live_host_login": "Connecte-toi en tant qu'administrateur pour animer une session.",
    "deck_empty": "Aucune activité. Choisis un type ci-dessus et ajoutes-en une.",
    "deck_title": "Titre de la présentation",
    "deck_title_ph": "ex. Atelier Atlantykron — retours",
    "deck_slides": "Activités",
    "deck_add_new": "Ajouter une activité",
    "live_prompt": "Question / sujet",
    ...
    "sv_title": "Titre du sondage",
    "sv_title_ph": "Ex : Retour événement ASFAN",
    ...
    "live_join_code": "Rejoindre avec un code",
    "live_code_ph": "CODE",
    "live_join": "Rejoindre",
    "live_host_new": "Ou lance une nouvelle activité",
    "deck_imported": (n)=>`${n} activités importées.`,
    "deck_full": "Maximum 50 activités.",
    "deck_added": "Activité ajoutée.",
    "deck_need_slides": "Ajoute au moins une activité.",
    "deck_default_title": "Présentation",
    "deck_import_bad": "Fichier invalide.",
    "deck_import_empty": "Aucune activité valide dans le fichier.",
    "game_waiting_players": "En attente de joueurs… partage le code !",
    "game_players": "joueurs",
    "game_start": "Commencer",
    "gh_typing": "Les participants écrivent…",
    "game_reveal": "Révéler la réponse",
    "gh_gotit": "correct",
    "game_results": "Résultats",
    "game_next": "Suivant",
    "team_standings": "Classement des équipes",
    "game_replay": "Rejouer",
    "game_report": "Rapport",
    "game_need_name": "Saisis un pseudo.",
    "game_lobby_closed": "La partie a déjà commencé.",
    "game_times_up": "Temps écoulé !",
    "game_pick_name": "Choisis un nom et un avatar",
    "game_nickname": "Ton pseudo",
    "nick_gen": "Pseudo aléatoire",
    "team_pick": "Choisis ton équipe",
    "game_enter": "Entrer dans le jeu",
    "game_youre_in": "Tu es dans la partie !",
    "game_look_screen": "Regarde l'écran principal.",
    "game_locked": "Réponse envoyée !",
    "game_wait_others": "En attente des autres joueurs…",
    "game_correct": "Correct !",
    "game_wrong": "Faux",
    "game_missed": "Pas de réponse",
    "game_your_rank": "Ton rang :",
    "game_finished": "Partie terminée !",
    "report_back": "Retour",
    "report_no_data": "Aucune donnée.",
    "report_players": "joueurs",
    "report_questions": "questions",
    "report_avg_score": "score moyen",
    "report_accuracy": "précision",
    "report_avg_time": "temps moyen",
    "report_easy": "facile",
    "report_med": "moyen",
    "report_hard": "difficile",
    "report_correct": "correctes",
    "report_answered": "répondu",
    "report_player": "Joueur",
    "report_csv": "CSV",
    "report_json": "JSON",
    "report_print": "Imprimer",
    "report_title": "Rapport de partie",
    "report_byq": "Par question",
    "report_byp": "Par joueur",
    "assign_none_done": "Personne n'a encore terminé.",
    "assign_done": "terminé",
    "assign_joined": "inscrits",
    "assign_closed": "Ce devoir est fermé.",
    "assign_closed_note": "L'animateur a fermé le devoir.",
    "assign_intro": (n)=>`${n} questions · à ton rythme`,
    "assign_start": "Commencer",
    "assign_finish": "Voir le résultat",
    "assign_complete": "Tu as terminé !",
    "offline_feature": "Indisponible hors ligne.",
    ...
    "install_hint": "Installer Undava comme appli",
    "install_btn": "Installer",
    "online_back": "Tu es de nouveau en ligne.",
    "live_clear_q": "Supprimer toutes les réponses ?",
    "live_end_q": "Terminer définitivement la session ? Les données seront supprimées."
  },
  "it": {
    "print_quiz": "Stampa",
    "unsaved_q": "Hai modifiche non salvate. Uscire senza salvare?",
    "review_btn": "Riepilogo",
    "review_title": "Riepilogo risposte",
    "review_answer": "Corretto:",
    "review_back": "Torna al punteggio",
    "sort_by": "Ordina",
    "sort_default": "Predefinito",
    "sort_name": "Nome A-Z",
    "sort_questions": "N. domande",
    "dup_q": "Duplica la domanda",
    "lib_search": "Cerca un quiz…",
    "lib_nomatch": "Nessun quiz trovato.",
    "tagline": "Quiz senza frontiere",
    "set_sound": "Suono",
    "settings": "Impostazioni",
    "spin_need": "Aggiungi almeno 2 elementi.",
    "back_home": "Home",
    "spin_title": "Ruota della fortuna",
    "spin_sub": "Inserisci nomi o opzioni, poi gira per scegliere a caso.",
    "spin_go": "Gira",
    "spin_items": "Elementi",
    "spin_ph": "Un elemento per riga…\nAlice\nBob\nCarla",
    "spin_elim": "Rimuovi il vincitore dopo ogni giro",
    "spin_shuffle": "Mescola",
    "spin_reset": "Reimposta",
    "spin_default": "Alice|Bob|Carla|Davide|Eva|Franco",
    "help_book_eli5": "Guida rapida",
    "help_book_user": "Manuale utente",
    "help_book_admin": "Manuale amministratore",
    "help_title": "Guida d'uso",
    "help_sub": "Manuale completo, passo dopo passo.",
    ...
    "feat_offline": "100% offline",
    "feat_free": "Trade-free",
    "feat_priv": "Zero telemetria",
    "feat_open": "File unico",
    "tf_desc": "senza pubblicità, senza raccolta dati, senza account, senza funzioni bloccate a pagamento",
    "tf_tools": "altri strumenti",
    ...
    "export": "Esporta",
    "del": "Elimina",
    "lib_title": "Libreria dei quiz",
    "lib_sub": "Scegli un quiz da giocare o modificare",
    "lib_mine": "I miei quiz",
    "export_all": "Esporta tutto",
    "lib_empty": "Non hai ancora creato nessun quiz. Tocca « Crea quiz » per iniziare.",
    "lib_samples": "Quiz di esempio",
    "library": "I miei quiz",
    "new_quiz": "Nuovo quiz",
    "edit_quiz": "Modifica quiz",
    "quiz_title": "Titolo del quiz",
    "quiz_title_ph": "es. Cultura generale",
    "quiz_desc": "Descrizione (facoltativa)",
    "quiz_desc_ph": "Una breve descrizione…",
    "questions": "Domande",
    "add_question": "Aggiungi domanda",
    "cancel": "Annulla",
    "save": "Salva quiz",
    "acc_primary_ph": "La risposta corretta (mostrata)",
    "acc_alt_ph": "Variante accettata (sinonimo, ortografia)",
    "acc_add": "Aggiungi variante accettata",
    "acc_hint": "Sono accettati anche piccoli errori di battitura. La prima variante è mostrata come risposta corretta.",
    "num_target": "Numero corretto",
    "num_target_ph": "es. 42",
    "num_tol": "Tolleranza accettata",
    ...
    "question_n": (n)=>"Domanda "+n,
    "type_quiz": "Scelta multipla (4)",
    "type_tf": "Vero / Falso",
    "type_type": "Risposta libera",
    "type_num": "Numerica (indovina)",
    "sec": "s",
    "pts_std": "Standard",
    "pts_dbl": "Doppio",
    "question_ph": "Scrivi qui la domanda…",
    "choose_mode": "Modalità di gioco",
    "mode_solo": "Solo",
    "mode_solo_d": "Gioca da solo e batti il tuo record.",
    "mode_hot": "A turni",
    "mode_hot_d": "Più giocatori sullo stesso dispositivo.",
    "shuffle_opt": "Mescola domande e risposte",
    "players": "Giocatori",
    "add_player": "+ Aggiungi giocatore",
    "start_game": "Inizia partita",
    "your_name": "Il tuo nome",
    "player_ph": "Nome del giocatore",
    "import_copied": "Copiato!",
    "import_title": "Importa un quiz",
    "import_drop": "Trascina qui un file .json o tocca per scegliere",
    "import_paste": "…oppure incolla il contenuto JSON:",
    "import_paste_ph": "{\"title\":\"…\",\"questions\":[…]}",
    "import_btn": "Importa",
    "import_fmt_title": "Formato JSON — crea a mano",
    "import_fmt_intro": "Puoi scrivere un file .json con la struttura qui sotto e importarlo qui. Ecco un esempio con tutti i tipi di domanda:",
    "import_copy": "Copia l'esempio",
    "import_template": "Scarica modello",
    "import_fields": "STRUTTURA DEL FILE\n• title — il nome del quiz\n• desc — breve descrizione (facoltativo)\n• color — colore hex, es. #2f6bff (facoltativo)\n• questions — l'elenco delle domande\n\nOGNI DOMANDA\n• type — quiz (scelte) / tf (vero-falso) / type (risposta scritta) / num (numerico)\n• text — il testo della domanda\n• time — secondi: 5, 10, 20, 30, 60 o 120\n• points — 1000 (standard) o 2000 (doppio)\n• answers — le risposte (vedi sotto)\n• tol — solo num: tolleranza accettata (±)\n• explain — spiegazione facoltativa (perché è corretto)\n\nRISPOSTE, PER TIPO\n• quiz: 2–4 scelte; segna quella giusta con \"correct\": true\n• tf: 2 scelte (Vero / Falso); segna quella giusta\n• type: 1–6 testi accettati (tutti corretti; aggiungi varianti)\n• num: un solo numero; tol facoltativo per la tolleranza",
    "import_ok": "Quiz importato!",
    "import_err": "File non valido. Controlla il formato JSON.",
    "you": "Tu",
    "need_player": "Aggiungi almeno un giocatore.",
    "scoreboard": "Classifica",
    "q_of": (a,b)=>"Domanda "+a+" di "+b,
    "pass_to": "Passa il dispositivo a",
    "tap_start": "Tocca quando sei pronto",
    "ready_q": "Pronto?",
    "num_answer_ph": "Scrivi un numero…",
    "type_answer_ph": "Scrivi la risposta…",
    "type_submit": "Invia",
    "keys_hint": "Premi 1-4 o tocca una risposta",
    "verdict_ok": "Corretto!",
    "verdict_no": "Sbagliato!",
    "verdict_miss": "Nessuna risposta",
    "correct_was": "Risposta corretta:",
    "you_wrote": "Hai scritto:",
    "streak": (n)=>n+" di fila 🔥",
    "continue": "Continua",
    "fastest5": "I più veloci",
    "stat_pts": "Punti",
    "stat_correct": "Corrette",
    "stat_acc": "Precisione",
    "stat_streak": "Serie migliore",
    "play_again": "Gioca ancora",
    "podium_title": "Congratulazioni!",
    "winner_is": "Vincitore",
    "type_need_answer": "Scrivi una risposta.",
    "set_title": "Impostazioni",
    "set_lang": "Lingua",
    "set_lang_d": "Lingua dell'interfaccia",
    "set_sound_d": "Effetti sonori durante il gioco",
    ...
    "backup_done": "Backup salvato",
    "quit_q": "Uscire dalla partita? I progressi andranno persi.",
    "err_title": "Dai un titolo al quiz.",
    "err_noq": "Aggiungi almeno una domanda.",
    "err_qtext": (n)=>"La domanda "+n+" non ha testo.",
    "err_acc": (n)=>`Domanda ${n}: aggiungi almeno una risposta accettata.`,
    "err_num": (n)=>`Domanda ${n}: inserisci un numero valido.`,
    "err_ans": (n)=>"La domanda "+n+" richiede almeno 2 risposte.",
    "err_corr": (n)=>"Indica la risposta corretta per la domanda "+n+".",
    "explain_ph": "Spiegazione (facoltativo) — perché è corretto",
    "net_err": "Errore di rete. Controlla la connessione.",
    "live_t_cloud": "Nuvola di parole",
    "live_t_cloud_d": "Il pubblico invia parole, mostrate dal vivo.",
    "live_t_poll": "Sondaggio dal vivo",
    "live_t_poll_d": "Vota le opzioni, barre in tempo reale.",
    "live_t_qa": "Domande e idee",
    "live_t_qa_d": "Messaggi del pubblico, con voti.",
    "live_t_rating": "Voto / NPS",
    "live_t_rating_d": "Voto a stelle o punteggio NPS 0–10.",
    "live_t_rank": "Classifica",
    "live_t_rank_d": "I partecipanti ordinano le opzioni per preferenza.",
    "live_t_scale": "Scale (Likert)",
    "live_t_scale_d": "Più affermazioni valutate da 1 a 5 (disaccordo→accordo).",
    "live_t_points": "100 punti",
    "live_t_points_d": "I partecipanti dividono un budget di punti tra le opzioni.",
    "cat_quiz": "Quiz & giochi",
    "cat_quiz_d": "Scelta multipla, vero/falso, risposta libera, numerico · Solo, a turni, dal vivo, compito o squadre",
    "cat_audience": "Attività per il pubblico",
    "cat_audience_d": "Raccogli risposte dal vivo dal pubblico, dai telefoni. Scegli un tipo:",
    "cat_deck": "Presentazioni",
    "cat_deck_d": "Concatena più attività in un'unica sessione, al tuo ritmo.",
    "deck_build": "Crea una presentazione",
    "cat_survey": "Sondaggio di feedback",
    "cat_tools": "Strumenti",
    "live_nav": "Dal vivo",
    "guestbook": "Feedback",
    "mode_live": "Dal vivo (multigiocatore)",
    "mode_live_d": "Ospita sul grande schermo; il pubblico gioca sui telefoni.",
    "mode_assign": "A proprio ritmo",
    "mode_assign_d": "Assegna un quiz; ognuno lo risolve da solo, al proprio ritmo.",
    "team_opt": "Modalità squadre",
    "team_word": "squadre",
    "assign_note": "I partecipanti entrano dai loro telefoni e risolvono al proprio ritmo. I risultati si raccolgono qui.",
    "game_join_note": "I giocatori entrano dai loro telefoni scansionando il QR code o inserendo il codice mostrato.",
    "game_host": "Ospita dal vivo",
    "assign_host": "Assegna compito",
    "fb_leave": "Lascia un feedback",
    "set_admin": "Amministratore",
    "set_admin_on": "Hai effettuato l'accesso come amministratore.",
    "set_admin_off": "Accedi per modificare i quiz.",
    "logout": "Esci",
    "login": "Accedi",
    "password": "Password",
    "password_again": "Ripeti la password",
    "fb_need_rating": "Scegli un voto (1–5 stelle).",
    "fb_need_msg": "Scrivi un messaggio.",
    "fb_thanks_mod": "Grazie! Il tuo messaggio apparirà dopo l'approvazione.",
    "fb_thanks": "Grazie per il tuo feedback!",
    "err_pw": "Inserisci la password.",
    "admin_ok": "Accesso effettuato!",
    "admin_bad": "Password errata.",
    "err_pw_short": "La password deve avere almeno 6 caratteri.",
    "err_pw_match": "Le password non coincidono.",
    "admin_out": "Disconnesso.",
    "fb_empty": "Ancora nessun feedback. Sii il primo!",
    "fb_title": "Feedback dei visitatori",
    "fb_sub": "Lascia un pensiero sui quiz e leggi cosa hanno scritto gli altri.",
    "fb_recent": "Feedback recenti",
    "fb_local_note": "(salvato localmente, su questo dispositivo)",
    "fb_pick_quiz": "— scegli un quiz —",
    "fb_name": "Nome (facoltativo)",
    "fb_name_ph": "Come ti chiamiamo?",
    "fb_quiz": "Quiz (facoltativo)",
    "fb_rating": "Voto",
    "fb_msg": "Messaggio",
    "fb_msg_ph": "Cosa ti è piaciuto? Cosa si può migliorare?",
    "fb_send": "Invia feedback",
    "fb_anon": "Anonimo",
    "fb_pending": "in attesa",
    "fb_approve": "Approva",
    "qa_pending": "In attesa",
    "qa_answered": "Risposto",
    "qa_approve": "Approva",
    "qa_star": "Evidenzia",
    "qa_mark": "Segna come risposto",
    "live_show": "Mostra",
    "live_hide": "Nascondi",
    "qa_pending_count": (n)=>`${n} in attesa`,
    "live_waiting": "In attesa di risposte…",
    "live_responses": "risposte",
    "rt_prom": "Promotori",
    "rt_pass": "Passivi",
    "rt_det": "Detrattori",
    "rt_avg": "media",
    "rk_foot": "ordinato per posizione media (più basso = meglio)",
    "scale_lo": "totale disaccordo",
    "scale_hi": "totale accordo",
    "pt_budget_each": (n)=>`${n} punti ciascuno`,
    "live_open": "APERTO",
    "live_closed_b": "CHIUSO",
    "live_need_prompt": "Scrivi una domanda o un argomento.",
    "live_need_opts": "Aggiungi almeno 2 opzioni.",
    "live_need_stmts": "Aggiungi almeno 2 affermazioni.",
    "sv_need_q": "Scrivi almeno una domanda.",
    "live_ended": "Sessione chiusa.",
    "live_copied": "Link copiato!",
    "live_json_name": "risultati-live",
    "live_png_name": "nuvola-di-parole",
    "live_back": "Chiudi",
    "live_mode_survey": "Sondaggio",
    "live_pause": "Pausa",
    "live_resume": "Riprendi",
    "live_clear": "Cancella",
    "live_end": "Termina",
    "live_join_at": "Entra su",
    "live_copy": "Copia link",
    "deck_prev": "Precedente",
    "deck_slide": "Attività",
    "deck_next": "Successivo",
    "live_share": "Condividi",
    "live_need_code": "Inserisci un codice.",
    "live_notfound": "Sessione non trovata o terminata.",
    "live_closed": "Le risposte sono chiuse.",
    "live_need_text": "Scrivi prima qualcosa.",
    "live_sent_mod": "Inviato per approvazione.",
    "live_sent": "Inviato!",
    "live_slow": "Troppo veloce — aspetta un attimo.",
    "scale_all": "Valuta ogni affermazione.",
    "pt_useall": "Usa tutti i tuoi punti",
    "sv_need_ans": "Scrivi almeno una risposta.",
    "sv_thanks": "Grazie! La tua risposta è stata inviata.",
    "live_leave": "Esci",
    "sv_word_ph": "Una parola o una frase breve…",
    "live_closed_note": "L'organizzatore ha sospeso le risposte.",
    "sv_send": "Invia",
    "live_loading": "Caricamento…",
    "live_voted": "Voto registrato!",
    "live_qa_ph": "Scrivi la tua domanda o idea…",
    "live_send": "Invia",
    "nps_low": "Per niente probabile",
    "nps_high": "Estremamente probabile",
    "rank_submit": "Invia classifica",
    "pt_remaining": "Rimanenti:",
    "live_word_ph": "Scrivi una parola…",
    "live_words_left": (n)=>`Puoi inviare ancora ${n} parola/e.`,
    "live_thanks_words": "Grazie! Guarda la nuvola di parole qui sotto.",
    "live_live_results": "Risultati dal vivo",
    "live_options": "Opzioni",
    "live_option": "Opzione",
    "live_add_option": "Aggiungi opzione",
    "live_rank_opts": "Opzioni da ordinare",
    "live_rating_scale": "Tipo di scala",
    "live_statements": "Affermazioni",
    "live_statement": "Affermazione",
    "live_add_stmt": "Aggiungi affermazione",
    "live_scale_hint": "Ogni affermazione è valutata su una scala da 1 (totale disaccordo) a 5 (totale accordo).",
    "live_points_opts": "Opzioni",
    "live_points_hint": "Ogni partecipante divide 100 punti tra queste opzioni.",
    "live_qa_mod": "Moderazione domande",
    "live_qa_mod_off": "Off (tutte visibili)",
    "live_qa_mod_on": "Richiedi approvazione",
    "live_qa_mod_hint": "Con l'approvazione, le domande appaiono al pubblico solo dopo che le approvi.",
    "live_filter": "Filtro volgarità",
    "live_filter_off": "Off",
    "live_filter_on": "On",
    "live_filter_hint": "Maschera automaticamente le parole volgari nelle risposte (le sostituisce con ***).",
    "live_max_words": "Quante parole può inviare una persona",
    "live_mode_single": "Attività singola",
    "live_mode_deck": "Presentazione",
    "live_host_login": "Accedi come amministratore per ospitare una sessione.",
    "deck_empty": "Ancora nessuna attività. Scegli un tipo qui sopra e aggiungine una.",
    "deck_title": "Titolo della presentazione",
    "deck_title_ph": "es. Workshop Atlantykron — feedback",
    "deck_slides": "Attività",
    "deck_add_new": "Aggiungi un'attività",
    "live_prompt": "Domanda / argomento",
    ...
    "sv_title": "Titolo del sondaggio",
    "sv_title_ph": "Es: Feedback evento ASFAN",
    ...
    "live_join_code": "Entra con un codice",
    "live_code_ph": "CODICE",
    "live_join": "Entra",
    "live_host_new": "Oppure avvia una nuova attività",
    "deck_imported": (n)=>`${n} attività importate.`,
    "deck_full": "Massimo 50 attività.",
    "deck_added": "Attività aggiunta.",
    "deck_need_slides": "Aggiungi almeno un'attività.",
    "deck_default_title": "Presentazione",
    "deck_import_bad": "File non valido.",
    "deck_import_empty": "Nessuna attività valida nel file.",
    "game_waiting_players": "In attesa di giocatori… condividi il codice!",
    "game_players": "giocatori",
    "game_start": "Inizia partita",
    "gh_typing": "I partecipanti stanno scrivendo…",
    "game_reveal": "Rivela risposta",
    "gh_gotit": "corrette",
    "game_results": "Risultati",
    "game_next": "Avanti",
    "team_standings": "Classifica squadre",
    "game_replay": "Gioca ancora",
    "game_report": "Report",
    "game_need_name": "Inserisci un nickname.",
    "game_lobby_closed": "La partita è già iniziata.",
    "game_times_up": "Tempo scaduto!",
    "game_pick_name": "Scegli nome e avatar",
    "game_nickname": "Il tuo nickname",
    "nick_gen": "Nickname casuale",
    "team_pick": "Scegli la tua squadra",
    "game_enter": "Entra nel gioco",
    "game_youre_in": "Sei dentro!",
    "game_look_screen": "Guarda lo schermo principale.",
    "game_locked": "Risposta inviata!",
    "game_wait_others": "In attesa degli altri giocatori…",
    "game_correct": "Corretto!",
    "game_wrong": "Sbagliato",
    "game_missed": "Nessuna risposta",
    "game_your_rank": "La tua posizione:",
    "game_finished": "Partita finita!",
    "report_back": "Indietro",
    "report_no_data": "Ancora nessun dato.",
    "report_players": "giocatori",
    "report_questions": "domande",
    "report_avg_score": "punteggio medio",
    "report_accuracy": "precisione",
    "report_avg_time": "tempo medio",
    "report_easy": "facile",
    "report_med": "media",
    "report_hard": "difficile",
    "report_correct": "corrette",
    "report_answered": "risposto",
    "report_player": "Giocatore",
    "report_csv": "CSV",
    "report_json": "JSON",
    "report_print": "Stampa",
    "report_title": "Report della partita",
    "report_byq": "Per domanda",
    "report_byp": "Per giocatore",
    "assign_none_done": "Nessuno ha ancora finito.",
    "assign_done": "finito",
    "assign_joined": "iscritti",
    "assign_closed": "Questo compito è chiuso.",
    "assign_closed_note": "L'organizzatore ha chiuso il compito.",
    "assign_intro": (n)=>`${n} domande · al tuo ritmo`,
    "assign_start": "Inizia",
    "assign_finish": "Vedi il risultato",
    "assign_complete": "Hai finito!",
    "offline_feature": "Non disponibile offline.",
    "offline_banner": "Sei offline — solo, libreria ed editor funzionano. Le modalità dal vivo tornano una volta riconnesso.",
    "install_hint": "Installa Undava come app",
    "install_btn": "Installa",
    "online_back": "Sei di nuovo online.",
    "live_clear_q": "Eliminare tutte le risposte?",
    "live_end_q": "Terminare definitivamente la sessione? I dati verranno eliminati."
  },
  "es": {
    "print_quiz": "Imprimir",
    "unsaved_q": "Tienes cambios sin guardar. ¿Salir sin guardar?",
    "review_btn": "Repaso",
    "review_title": "Repaso de respuestas",
    "review_answer": "Correcto:",
    "review_back": "Volver al marcador",
    "sort_by": "Ordenar",
    "sort_default": "Predeterminado",
    "sort_name": "Nombre A-Z",
    "sort_questions": "N.º preguntas",
    "dup_q": "Duplicar pregunta",
    "lib_search": "Buscar cuestionarios…",
    "lib_nomatch": "No se encontraron cuestionarios.",
    "tagline": "Quiz sin fronteras",
    "set_sound": "Sonido",
    "settings": "Ajustes",
    "spin_need": "Añade al menos 2 elementos.",
    "back_home": "Inicio",
    "spin_title": "Ruleta de la fortuna",
    "spin_sub": "Introduce nombres u opciones y gira para elegir al azar.",
    "spin_go": "Girar",
    "spin_items": "Elementos",
    "spin_ph": "Un elemento por línea…\nAna\nBeto\nCarla",
    "spin_elim": "Quitar al ganador tras cada giro",
    "spin_shuffle": "Mezclar",
    "spin_reset": "Reiniciar",
    "spin_default": "Ana|Beto|Carla|David|Eva|Fran",
    "help_book_eli5": "Guía rápida",
    "help_book_user": "Manual del usuario",
    "help_book_admin": "Manual del administrador",
    "help_title": "Guía de uso",
    "help_sub": "Manual completo, paso a paso.",
    ...
    "spin_nav": "Ruleta",
    "help_nav": "Guía",
    "feat_offline": "100% sin conexión",
    "feat_free": "Sin comercio",
    "feat_priv": "Cero telemetría",
    "feat_open": "Archivo único",
    "tf_desc": "sin anuncios, sin recopilación de datos, sin cuenta necesaria, sin funciones bloqueadas tras pago",
    "tf_tools": "otras herramientas",
    ...
    "export": "Exportar",
    "del": "Eliminar",
    "lib_title": "Biblioteca de cuestionarios",
    "lib_sub": "Elige un cuestionario para jugar o editar",
    "lib_mine": "Mis cuestionarios",
    "export_all": "Exportar todo",
    "lib_empty": "Aún no has creado ningún cuestionario. Toca « Crear cuestionario » para empezar.",
    "lib_samples": "Cuestionarios de ejemplo",
    "library": "Mis cuestionarios",
    "new_quiz": "Nuevo cuestionario",
    "edit_quiz": "Editar cuestionario",
    "quiz_title": "Título del cuestionario",
    "quiz_title_ph": "p. ej. Cultura general",
    "quiz_desc": "Descripción (opcional)",
    "quiz_desc_ph": "Una breve descripción…",
    "questions": "Preguntas",
    "add_question": "Añadir pregunta",
    "cancel": "Cancelar",
    "save": "Guardar cuestionario",
    "acc_primary_ph": "La respuesta correcta (mostrada)",
    "acc_alt_ph": "Variante aceptada (sinónimo, ortografía)",
    "acc_add": "Añadir variante aceptada",
    "acc_hint": "También se aceptan pequeños errores de tipeo. La primera variante se muestra como respuesta correcta.",
    "num_target": "Número correcto",
    "num_target_ph": "p. ej. 42",
    "num_tol": "Tolerancia aceptada",
    ...
    "question_n": (n)=>"Pregunta "+n,
    "type_quiz": "Opción múltiple (4)",
    "type_tf": "Verdadero / Falso",
    "type_type": "Respuesta libre",
    "type_num": "Numérica (adivina)",
    "sec": "s",
    "pts_std": "Estándar",
    "pts_dbl": "Doble",
    "question_ph": "Escribe aquí la pregunta…",
    "choose_mode": "Modo de juego",
    "mode_solo": "Solo",
    "mode_solo_d": "Juega solo y supera tu récord.",
    "mode_hot": "Por turnos",
    "mode_hot_d": "Varios jugadores en el mismo dispositivo.",
    "shuffle_opt": "Mezclar preguntas y respuestas",
    "players": "Jugadores",
    "add_player": "+ Añadir jugador",
    "start_game": "Empezar partida",
    "your_name": "Tu nombre",
    "player_ph": "Nombre del jugador",
    "import_copied": "¡Copiado!",
    "import_title": "Importar un cuestionario",
    "import_drop": "Suelta aquí un archivo .json o toca para elegir",
    "import_paste": "…o pega el contenido JSON:",
    "import_paste_ph": "{\"title\":\"…\",\"questions\":[…]}",
    "import_btn": "Importar",
    "import_fmt_title": "Formato JSON — crear a mano",
    "import_fmt_intro": "Puedes escribir un archivo .json con la estructura de abajo e importarlo aquí. Aquí tienes un ejemplo con todos los tipos de pregunta:",
    "import_copy": "Copiar ejemplo",
    "import_template": "Descargar plantilla",
    "import_fields": "ESTRUCTURA DEL ARCHIVO\n• title — el nombre del cuestionario\n• desc — descripción breve (opcional)\n• color — color hex, p. ej. #2f6bff (opcional)\n• questions — la lista de preguntas\n\nCADA PREGUNTA\n• type — quiz (opciones) / tf (verdadero-falso) / type (respuesta escrita) / num (numérico)\n• text — el enunciado\n• time — segundos: 5, 10, 20, 30, 60 o 120\n• points — 1000 (estándar) o 2000 (doble)\n• answers — las respuestas (ver abajo)\n• tol — solo num: tolerancia aceptada (±)\n• explain — explicación opcional (por qué es correcto)\n\nRESPUESTAS, POR TIPO\n• quiz: 2–4 opciones; marca la correcta con \"correct\": true\n• tf: 2 opciones (Verdadero / Falso); marca la correcta\n• type: 1–6 textos aceptados (todos correctos; añade variantes)\n• num: un solo número; tol opcional para la tolerancia",
    "import_ok": "¡Cuestionario importado!",
    "import_err": "Archivo no válido. Comprueba el formato JSON.",
    "you": "Tú",
    "need_player": "Añade al menos un jugador.",
    "scoreboard": "Clasificación",
    "q_of": (a,b)=>"Pregunta "+a+" de "+b,
    "pass_to": "Pasa el dispositivo a",
    "tap_start": "Toca cuando estés listo",
    "ready_q": "¿Listo?",
    "num_answer_ph": "Escribe un número…",
    "type_answer_ph": "Escribe tu respuesta…",
    "type_submit": "Enviar",
    "keys_hint": "Pulsa 1-4 o toca una respuesta",
    "verdict_ok": "¡Correcto!",
    "verdict_no": "¡Incorrecto!",
    "verdict_miss": "Sin respuesta",
    "correct_was": "Respuesta correcta:",
    "you_wrote": "Escribiste:",
    "streak": (n)=>n+" seguidas 🔥",
    "continue": "Continuar",
    "fastest5": "Los más rápidos",
    "stat_pts": "Puntos",
    "stat_correct": "Correctas",
    "stat_acc": "Precisión",
    "stat_streak": "Mejor racha",
    "play_again": "Jugar de nuevo",
    "podium_title": "¡Felicidades!",
    "winner_is": "Ganador",
    "type_need_answer": "Escribe una respuesta.",
    "set_title": "Ajustes",
    "set_lang": "Idioma",
    "set_lang_d": "Idioma de la interfaz",
    "set_sound_d": "Efectos de sonido durante el juego",
    ...
    "deleted": "Cuestionario eliminado",
    "backup_done": "Copia guardada",
    "quit_q": "¿Salir de la partida? Se perderá el progreso.",
    "err_title": "Ponle un título al cuestionario.",
    "err_noq": "Añade al menos una pregunta.",
    "err_qtext": (n)=>"La pregunta "+n+" no tiene texto.",
    "err_acc": (n)=>`Pregunta ${n}: añade al menos una respuesta aceptada.`,
    "err_num": (n)=>`Pregunta ${n}: introduce un número válido.`,
    "err_ans": (n)=>"La pregunta "+n+" necesita al menos 2 respuestas.",
    "err_corr": (n)=>"Marca la respuesta correcta de la pregunta "+n+".",
    "explain_ph": "Explicación (opcional) — por qué es correcto",
    "net_err": "Error de red. Comprueba tu conexión.",
    "live_t_cloud": "Nube de palabras",
    "live_t_cloud_d": "El público envía palabras, mostradas en directo.",
    "live_t_poll": "Encuesta en directo",
    "live_t_poll_d": "Vota opciones, barras en tiempo real.",
    "live_t_qa": "Preguntas e ideas",
    "live_t_qa_d": "Mensajes del público, con votos.",
    "live_t_rating": "Valoración / NPS",
    "live_t_rating_d": "Valoración por estrellas o puntuación NPS 0–10.",
    "live_t_rank": "Ranking",
    "live_t_rank_d": "Los participantes ordenan las opciones por preferencia.",
    "live_t_scale": "Escalas (Likert)",
    "live_t_scale_d": "Varias afirmaciones valoradas de 1 a 5 (desacuerdo→acuerdo).",
    "live_t_points": "100 puntos",
    "live_t_points_d": "Los participantes reparten un presupuesto de puntos entre las opciones.",
    "cat_quiz": "Cuestionarios & juegos",
    "cat_quiz_d": "Opción múltiple, verdadero/falso, respuesta libre, numérico · Solo, por turnos, en directo, tarea o equipos",
    "cat_audience": "Actividades de audiencia",
    "cat_audience_d": "Recoge respuestas en directo del público, desde sus teléfonos. Elige un tipo:",
    "cat_deck": "Presentaciones",
    "cat_deck_d": "Encadena varias actividades en una sola sesión, a tu ritmo.",
    "deck_build": "Crear una presentación",
    "cat_survey": "Cuestionario de feedback",
    "cat_tools": "Herramientas",
    "live_nav": "En directo",
    "guestbook": "Comentarios",
    "mode_live": "En directo (multijugador)",
    "mode_live_d": "Dirige en la pantalla grande; el público juega en móviles.",
    "mode_assign": "A tu ritmo",
    "mode_assign_d": "Asigna un cuestionario; cada uno lo resuelve por su cuenta, a su ritmo.",
    "team_opt": "Modo equipos",
    "team_word": "equipos",
    "assign_note": "Los participantes entran desde sus móviles y resuelven a su ritmo. Los resultados se recogen aquí.",
    "game_join_note": "Los jugadores entran desde sus móviles escaneando el código QR o introduciendo el código mostrado.",
    "game_host": "Dirigir en directo",
    "assign_host": "Asignar tarea",
    "fb_leave": "Dejar un comentario",
    "set_admin": "Administrador",
    "set_admin_on": "Has iniciado sesión como administrador.",
    "set_admin_off": "Inicia sesión para editar cuestionarios.",
    "logout": "Cerrar sesión",
    "login": "Iniciar sesión",
    "password": "Contraseña",
    "password_again": "Repite la contraseña",
    "fb_need_rating": "Elige una valoración (1–5 estrellas).",
    "fb_need_msg": "Escribe un mensaje.",
    "fb_thanks_mod": "¡Gracias! Tu mensaje aparecerá tras la aprobación.",
    "fb_thanks": "¡Gracias por tu comentario!",
    "err_pw": "Introduce la contraseña.",
    "admin_ok": "¡Sesión iniciada!",
    ...
    "admin_out": "Sesión cerrada.",
    "fb_empty": "Aún no hay comentarios. ¡Sé el primero!",
    "fb_title": "Comentarios de los visitantes",
    "fb_sub": "Deja una opinión sobre los cuestionarios y mira lo que escribieron otros.",
    "fb_recent": "Comentarios recientes",
    "fb_local_note": "(guardado localmente, en este dispositivo)",
    "fb_pick_quiz": "— elige un cuestionario —",
    "fb_name": "Nombre (opcional)",
    "fb_name_ph": "¿Cómo te llamamos?",
    "fb_quiz": "Cuestionario (opcional)",
    "fb_rating": "Valoración",
    "fb_msg": "Mensaje",
    "fb_msg_ph": "¿Qué te gustó? ¿Qué se podría mejorar?",
    "fb_send": "Enviar comentario",
    "fb_anon": "Anónimo",
    "fb_pending": "pendiente",
    "fb_approve": "Aprobar",
    "qa_pending": "Pendiente",
    "qa_answered": "Respondida",
    "qa_approve": "Aprobar",
    "qa_star": "Destacar",
    "qa_mark": "Marcar como respondida",
    "live_show": "Mostrar",
    "live_hide": "Ocultar",
    "qa_pending_count": (n)=>`${n} pendientes`,
    "live_waiting": "Esperando respuestas…",
    "live_responses": "respuestas",
    "rt_prom": "Promotores",
    "rt_pass": "Pasivos",
    "rt_det": "Detractores",
    "rt_avg": "media",
    "rk_foot": "ordenado por puesto medio (menor = mejor)",
    "scale_lo": "muy en desacuerdo",
    "scale_hi": "muy de acuerdo",
    "pt_budget_each": (n)=>`${n} puntos cada uno`,
    "live_open": "ABIERTA",
    "live_closed_b": "CERRADA",
    "live_need_prompt": "Escribe una pregunta o un tema.",
    "live_need_opts": "Añade al menos 2 opciones.",
    "live_need_stmts": "Añade al menos 2 afirmaciones.",
    "sv_need_q": "Escribe al menos una pregunta.",
    "live_ended": "Sesión cerrada.",
    "live_copied": "¡Enlace copiado!",
    "live_json_name": "resultados-live",
    "live_png_name": "nube-de-palabras",
    "live_back": "Cerrar",
    "live_mode_survey": "Cuestionario",
    "live_pause": "Pausar",
    "live_resume": "Reanudar",
    "live_clear": "Borrar",
    "live_end": "Finalizar",
    "live_join_at": "Únete en",
    "live_copy": "Copiar enlace",
    "deck_prev": "Anterior",
    "deck_slide": "Actividad",
    "deck_next": "Siguiente",
    "live_share": "Compartir",
    "live_need_code": "Introduce un código.",
    "live_notfound": "Sesión no encontrada o finalizada.",
    "live_closed": "Las respuestas están cerradas.",
    "live_need_text": "Escribe algo primero.",
    "live_sent_mod": "Enviado para aprobación.",
    "live_sent": "¡Enviado!",
    "live_slow": "Demasiado rápido — espera un momento.",
    "scale_all": "Valora cada afirmación.",
    "pt_useall": "Usa todos tus puntos",
    "sv_need_ans": "Escribe al menos una respuesta.",
    "sv_thanks": "¡Gracias! Tu respuesta se ha enviado.",
    "live_leave": "Salir",
    "sv_word_ph": "Una palabra o una frase corta…",
    "live_closed_note": "El anfitrión pausó las respuestas.",
    "sv_send": "Enviar",
    "live_loading": "Cargando…",
    "live_voted": "¡Voto contado!",
    "live_qa_ph": "Escribe tu pregunta o idea…",
    "live_send": "Enviar",
    "nps_low": "Nada probable",
    "nps_high": "Extremadamente probable",
    "rank_submit": "Enviar ranking",
    "pt_remaining": "Restantes:",
    "live_word_ph": "Escribe una palabra…",
    "live_words_left": (n)=>`Puedes enviar ${n} palabra(s) más.`,
    "live_thanks_words": "¡Gracias! Mira la nube de palabras abajo.",
    "live_live_results": "Resultados en directo",
    "live_options": "Opciones",
    "live_option": "Opción",
    "live_add_option": "Añadir opción",
    "live_rank_opts": "Opciones para ordenar",
    "live_rating_scale": "Tipo de escala",
    "live_statements": "Afirmaciones",
    "live_statement": "Afirmación",
    "live_add_stmt": "Añadir afirmación",
    "live_scale_hint": "Cada afirmación se valora en una escala de 1 (muy en desacuerdo) a 5 (muy de acuerdo).",
    "live_points_opts": "Opciones",
    "live_points_hint": "Cada participante reparte 100 puntos entre estas opciones.",
    "live_qa_mod": "Moderación de preguntas",
    "live_qa_mod_off": "Off (se muestran todas)",
    "live_qa_mod_on": "Requerir aprobación",
    "live_qa_mod_hint": "Con aprobación, las preguntas aparecen al público solo después de que las apruebes.",
    "live_filter": "Filtro de palabrotas",
    "live_filter_off": "Off",
    "live_filter_on": "On",
    "live_filter_hint": "Enmascara automáticamente las palabras vulgares en las respuestas (las reemplaza por ***).",
    "live_max_words": "Cuántas palabras puede enviar una persona",
    "live_mode_single": "Actividad única",
    "live_mode_deck": "Presentación",
    "live_host_login": "Inicia sesión como administrador para dirigir una sesión.",
    "deck_empty": "Aún no hay actividades. Elige un tipo arriba y añade una.",
    "deck_title": "Título de la presentación",
    "deck_title_ph": "p. ej. Taller Atlantykron — opiniones",
    "deck_slides": "Actividades",
    "deck_add_new": "Añadir una actividad",
    "live_prompt": "Pregunta / tema",
    ...
    "sv_title": "Título del cuestionario",
    "sv_title_ph": "Ej: Feedback evento ASFAN",
    ...
    "live_join_code": "Únete con un código",
    "live_code_ph": "CÓDIGO",
    "live_join": "Unirse",
    "live_host_new": "O inicia una nueva actividad",
    "deck_imported": (n)=>`${n} actividades importadas.`,
    "deck_full": "Máximo 50 actividades.",
    "deck_added": "Actividad añadida.",
    "deck_need_slides": "Añade al menos una actividad.",
    "deck_default_title": "Presentación",
    "deck_import_bad": "Archivo no válido.",
    "deck_import_empty": "No hay actividades válidas en el archivo.",
    "game_waiting_players": "Esperando jugadores… ¡comparte el código!",
    "game_players": "jugadores",
    "game_start": "Empezar partida",
    "gh_typing": "Los participantes están escribiendo…",
    "game_reveal": "Revelar respuesta",
    "gh_gotit": "correctas",
    "game_results": "Resultados",
    "game_next": "Siguiente",
    "team_standings": "Clasificación por equipos",
    "game_replay": "Jugar de nuevo",
    "game_report": "Informe",
    "game_need_name": "Introduce un apodo.",
    "game_lobby_closed": "La partida ya empezó.",
    "game_times_up": "¡Se acabó el tiempo!",
    "game_pick_name": "Elige nombre y avatar",
    "game_nickname": "Tu apodo",
    "nick_gen": "Apodo aleatorio",
    "team_pick": "Elige tu equipo",
    "game_enter": "Entrar al juego",
    "game_youre_in": "¡Estás dentro!",
    "game_look_screen": "Mira la pantalla principal.",
    "game_locked": "¡Respuesta enviada!",
    "game_wait_others": "Esperando a los demás jugadores…",
    "game_correct": "¡Correcto!",
    "game_wrong": "Incorrecto",
    "game_missed": "Sin respuesta",
    "game_your_rank": "Tu puesto:",
    "game_finished": "¡Partida terminada!",
    "report_back": "Atrás",
    "report_no_data": "Aún no hay datos.",
    "report_players": "jugadores",
    "report_questions": "preguntas",
    "report_avg_score": "puntuación media",
    "report_accuracy": "precisión",
    "report_avg_time": "tiempo medio",
    "report_easy": "fácil",
    "report_med": "media",
    "report_hard": "difícil",
    "report_correct": "correctas",
    "report_answered": "respondido",
    "report_player": "Jugador",
    "report_csv": "CSV",
    "report_json": "JSON",
    "report_print": "Imprimir",
    "report_title": "Informe de la partida",
    "report_byq": "Por pregunta",
    "report_byp": "Por jugador",
    "assign_none_done": "Nadie ha terminado aún.",
    "assign_done": "terminado",
    "assign_joined": "inscritos",
    "assign_closed": "Esta tarea está cerrada.",
    "assign_closed_note": "El anfitrión cerró la tarea.",
    "assign_intro": (n)=>`${n} preguntas · a tu ritmo`,
    "assign_start": "Empezar",
    "assign_finish": "Ver resultado",
    "assign_complete": "¡Has terminado!",
    "offline_feature": "No disponible sin conexión.",
    "offline_banner": "Estás sin conexión — solo, biblioteca y editor funcionan. Los modos en directo vuelven al reconectar.",
    "install_hint": "Instalar Undava como app",
    "install_btn": "Instalar",
    "online_back": "Vuelves a estar en línea.",
    "live_clear_q": "¿Eliminar todas las respuestas?",
    "live_end_q": "¿Finalizar la sesión para siempre? Se eliminarán los datos."
  },
  "pt": {
    "print_quiz": "Imprimir",
    "unsaved_q": "Tens alterações não guardadas. Sair sem guardar?",
    "review_btn": "Recap",
    "review_title": "Revisão de respostas",
    "review_answer": "Correto:",
    "review_back": "Voltar ao placar",
    "sort_by": "Ordenar",
    "sort_default": "Predefinido",
    "sort_name": "Nome A-Z",
    "sort_questions": "N.º perguntas",
    "dup_q": "Duplicar pergunta",
    "lib_search": "Procurar um quiz…",
    "lib_nomatch": "Nenhum quiz encontrado.",
    "tagline": "Quiz sem fronteiras",
    "set_sound": "Som",
    "settings": "Definições",
    "spin_need": "Adiciona pelo menos 2 elementos.",
    "back_home": "Início",
    "spin_title": "Roda da sorte",
    "spin_sub": "Introduz nomes ou opções e roda para escolher ao acaso.",
    "spin_go": "Rodar",
    "spin_items": "Elementos",
    "spin_ph": "Um elemento por linha…\nAna\nBeto\nCarla",
    "spin_elim": "Remover o vencedor após cada rodada",
    "spin_shuffle": "Baralhar",
    "spin_reset": "Repor",
    "spin_default": "Ana|Beto|Carla|David|Eva|Rui",
    "help_book_eli5": "Guia rápido",
    "help_book_user": "Manual do utilizador",
    "help_book_admin": "Manual do administrador",
    "help_title": "Guia de utilização",
    "help_sub": "Manual completo, passo a passo.",
    ...
    "help_nav": "Guia",
    "feat_offline": "100% offline",
    "feat_free": "Sem comércio",
    "feat_priv": "Zero telemetria",
    "feat_open": "Ficheiro único",
    "tf_desc": "sem anúncios, sem recolha de dados, sem conta necessária, sem funções bloqueadas por pagamento",
    "tf_tools": "outras ferramentas",
    ...
    "export": "Exportar",
    "del": "Eliminar",
    "lib_title": "Biblioteca de questionários",
    "lib_sub": "Escolhe um questionário para jogar ou editar",
    "lib_mine": "Os meus questionários",
    "export_all": "Exportar tudo",
    "lib_empty": "Ainda não criaste nenhum questionário. Toca em « Criar questionário » para começar.",
    "lib_samples": "Questionários de exemplo",
    "library": "Os meus questionários",
    "new_quiz": "Novo questionário",
    "edit_quiz": "Editar questionário",
    "quiz_title": "Título do questionário",
    "quiz_title_ph": "ex. Cultura geral",
    "quiz_desc": "Descrição (opcional)",
    "quiz_desc_ph": "Uma breve descrição…",
    "questions": "Perguntas",
    "add_question": "Adicionar pergunta",
    "cancel": "Cancelar",
    "save": "Guardar questionário",
    "acc_primary_ph": "A resposta correta (mostrada)",
    "acc_alt_ph": "Variante aceite (sinónimo, ortografia)",
    "acc_add": "Adicionar variante aceite",
    "acc_hint": "Também são aceites pequenos erros de escrita. A primeira variante é mostrada como resposta correta.",
    "num_target": "Número correto",
    "num_target_ph": "ex. 42",
    "num_tol": "Tolerância aceite",
    ...
    "question_n": (n)=>"Pergunta "+n,
    "type_quiz": "Escolha múltipla (4)",
    "type_tf": "Verdadeiro / Falso",
    "type_type": "Resposta livre",
    "type_num": "Numérica (adivinha)",
    "sec": "s",
    "pts_std": "Padrão",
    "pts_dbl": "Duplo",
    "question_ph": "Escreve aqui a pergunta…",
    "choose_mode": "Modo de jogo",
    "mode_solo": "Solo",
    "mode_solo_d": "Joga sozinho e bate o teu recorde.",
    "mode_hot": "À vez",
    "mode_hot_d": "Vários jogadores no mesmo dispositivo.",
    "shuffle_opt": "Baralhar perguntas e respostas",
    "players": "Jogadores",
    "add_player": "+ Adicionar jogador",
    "start_game": "Começar jogo",
    "your_name": "O teu nome",
    "player_ph": "Nome do jogador",
    "import_copied": "Copiado!",
    "import_title": "Importar um questionário",
    "import_drop": "Larga aqui um ficheiro .json ou toca para escolher",
    "import_paste": "…ou cola o conteúdo JSON:",
    "import_paste_ph": "{\"title\":\"…\",\"questions\":[…]}",
    "import_btn": "Importar",
    "import_fmt_title": "Formato JSON — criar à mão",
    "import_fmt_intro": "Podes escrever um ficheiro .json com a estrutura abaixo e importá-lo aqui. Eis um exemplo com todos os tipos de pergunta:",
    "import_copy": "Copiar exemplo",
    "import_template": "Transferir modelo",
    "import_fields": "ESTRUTURA DO FICHEIRO\n• title — o nome do quiz\n• desc — descrição breve (opcional)\n• color — cor hex, ex. #2f6bff (opcional)\n• questions — a lista de perguntas\n\nCADA PERGUNTA\n• type — quiz (opções) / tf (verdadeiro-falso) / type (resposta escrita) / num (numérico)\n• text — o enunciado\n• time — segundos: 5, 10, 20, 30, 60 ou 120\n• points — 1000 (padrão) ou 2000 (duplo)\n• answers — as respostas (ver abaixo)\n• tol — só num: tolerância aceite (±)\n• explain — explicação opcional (porque está correto)\n\nRESPOSTAS, POR TIPO\n• quiz: 2–4 opções; marca a correta com \"correct\": true\n• tf: 2 opções (Verdadeiro / Falso); marca a correta\n• type: 1–6 textos aceites (todos corretos; adiciona variantes)\n• num: um único número; tol opcional para a tolerância",
    "import_ok": "Questionário importado!",
    "import_err": "Ficheiro inválido. Verifica o formato JSON.",
    "you": "Tu",
    "need_player": "Adiciona pelo menos um jogador.",
    "scoreboard": "Classificação",
    "q_of": (a,b)=>"Pergunta "+a+" de "+b,
    "pass_to": "Passa o dispositivo a",
    "tap_start": "Toca quando estiveres pronto",
    "ready_q": "Pronto?",
    "num_answer_ph": "Escreve um número…",
    "type_answer_ph": "Escreve a tua resposta…",
    "type_submit": "Enviar",
    "keys_hint": "Prime 1-4 ou toca numa resposta",
    "verdict_ok": "Correto!",
    "verdict_no": "Errado!",
    "verdict_miss": "Sem resposta",
    "correct_was": "Resposta correta:",
    "you_wrote": "Escreveste:",
    "streak": (n)=>n+" seguidas 🔥",
    "continue": "Continuar",
    "fastest5": "Os mais rápidos",
    "stat_pts": "Pontos",
    "stat_correct": "Corretas",
    "stat_acc": "Precisão",
    "stat_streak": "Melhor sequência",
    "play_again": "Jogar de novo",
    "podium_title": "Parabéns!",
    "winner_is": "Vencedor",
    "type_need_answer": "Escreve uma resposta.",
    "set_title": "Definições",
    "set_lang": "Idioma",
    "set_lang_d": "Idioma da interface",
    "set_sound_d": "Efeitos sonoros durante o jogo",
    ...
    "close": "Fechar",
    "saved": "Questionário guardado!",
    "confirm_del": "Eliminar este questionário para sempre?",
    "deleted": "Questionário eliminado",
    "backup_done": "Cópia guardada",
    "quit_q": "Sair do jogo? O progresso será perdido.",
    "err_title": "Dá um título ao questionário.",
    "err_noq": "Adiciona pelo menos uma pergunta.",
    "err_qtext": (n)=>"A pergunta "+n+" não tem texto.",
    "err_acc": (n)=>`Pergunta ${n}: adiciona pelo menos uma resposta aceite.`,
    "err_num": (n)=>`Pergunta ${n}: introduz um número válido.`,
    "err_ans": (n)=>"A pergunta "+n+" precisa de pelo menos 2 respostas.",
    "err_corr": (n)=>"Marca a resposta correta da pergunta "+n+".",
    "explain_ph": "Explicação (opcional) — porque está correto",
    "net_err": "Erro de rede. Verifica a tua ligação.",
    "live_t_cloud": "Nuvem de palavras",
    "live_t_cloud_d": "O público envia palavras, mostradas ao vivo.",
    "live_t_poll": "Sondagem ao vivo",
    "live_t_poll_d": "Vota opções, barras em tempo real.",
    "live_t_qa": "Perguntas e ideias",
    "live_t_qa_d": "Mensagens do público, com votos.",
    "live_t_rating": "Avaliação / NPS",
    "live_t_rating_d": "Avaliação por estrelas ou pontuação NPS 0–10.",
    "live_t_rank": "Ranking",
    "live_t_rank_d": "Os participantes ordenam as opções por preferência.",
    "live_t_scale": "Escalas (Likert)",
    "live_t_scale_d": "Várias afirmações avaliadas de 1 a 5 (desacordo→acordo).",
    "live_t_points": "100 pontos",
    "live_t_points_d": "Os participantes dividem um orçamento de pontos pelas opções.",
    "cat_quiz": "Questionários & jogos",
    "cat_quiz_d": "Escolha múltipla, verdadeiro/falso, resposta livre, numérico · Solo, à vez, ao vivo, trabalho ou equipas",
    "cat_audience": "Atividades de audiência",
    "cat_audience_d": "Recolhe respostas ao vivo do público, dos telemóveis. Escolhe um tipo:",
    "cat_deck": "Apresentações",
    "cat_deck_d": "Encadeia várias atividades numa única sessão, ao teu ritmo.",
    "deck_build": "Criar uma apresentação",
    "cat_survey": "Questionário de feedback",
    "cat_tools": "Ferramentas",
    "live_nav": "Ao vivo",
    "guestbook": "Comentários",
    "mode_live": "Ao vivo (multijogador)",
    "mode_live_d": "Dinamiza no ecrã grande; o público joga nos telemóveis.",
    "mode_assign": "Ao teu ritmo",
    "mode_assign_d": "Atribui um questionário; cada um resolve sozinho, ao seu ritmo.",
    "team_opt": "Modo equipas",
    "team_word": "equipas",
    "assign_note": "Os participantes entram pelos telemóveis e resolvem ao seu ritmo. Os resultados juntam-se aqui.",
    "game_join_note": "Os jogadores entram pelos telemóveis lendo o código QR ou introduzindo o código mostrado.",
    "game_host": "Dinamizar ao vivo",
    "assign_host": "Atribuir trabalho",
    "fb_leave": "Deixar um comentário",
    "set_admin": "Administrador",
    "set_admin_on": "Tens sessão iniciada como administrador.",
    "set_admin_off": "Inicia sessão para editar questionários.",
    "logout": "Terminar sessão",
    "login": "Iniciar sessão",
    "password": "Palavra-passe",
    "password_again": "Repete a palavra-passe",
    "fb_need_rating": "Escolhe uma avaliação (1–5 estrelas).",
    "fb_need_msg": "Escreve uma mensagem.",
    "fb_thanks_mod": "Obrigado! A tua mensagem aparecerá após aprovação.",
    "fb_thanks": "Obrigado pelo teu comentário!",
    "err_pw": "Introduz a palavra-passe.",
    "admin_ok": "Sessão iniciada!",
    "admin_bad": "Palavra-passe incorreta.",
    "err_pw_short": "A palavra-passe deve ter pelo menos 6 caracteres.",
    "err_pw_match": "As palavras-passe não coincidem.",
    "admin_out": "Sessão terminada.",
    "fb_empty": "Ainda não há comentários. Sê o primeiro!",
    "fb_title": "Comentários dos visitantes",
    "fb_sub": "Deixa uma opinião sobre os questionários e vê o que os outros escreveram.",
    "fb_recent": "Comentários recentes",
    "fb_local_note": "(guardado localmente, neste dispositivo)",
    "fb_pick_quiz": "— escolhe um questionário —",
    "fb_name": "Nome (opcional)",
    "fb_name_ph": "Como te chamamos?",
    "fb_quiz": "Questionário (opcional)",
    "fb_rating": "Avaliação",
    "fb_msg": "Mensagem",
    "fb_msg_ph": "Do que gostaste? O que se poderia melhorar?",
    "fb_send": "Enviar comentário",
    "fb_anon": "Anónimo",
    "fb_pending": "pendente",
    "fb_approve": "Aprovar",
    "qa_pending": "Pendente",
    ...
    "live_show": "Mostrar",
    "live_hide": "Ocultar",
    "qa_pending_count": (n)=>`${n} pendentes`,
    "live_waiting": "À espera de respostas…",
    "live_responses": "respostas",
    "rt_prom": "Promotores",
    "rt_pass": "Passivos",
    "rt_det": "Detratores",
    "rt_avg": "média",
    "rk_foot": "ordenado por posição média (menor = melhor)",
    "scale_lo": "discordo totalmente",
    "scale_hi": "concordo totalmente",
    "pt_budget_each": (n)=>`${n} pontos cada`,
    "live_open": "ABERTA",
    "live_closed_b": "FECHADA",
    "live_need_prompt": "Escreve uma pergunta ou um tema.",
    "live_need_opts": "Adiciona pelo menos 2 opções.",
    "live_need_stmts": "Adiciona pelo menos 2 afirmações.",
    "sv_need_q": "Escreve pelo menos uma pergunta.",
    "live_ended": "Sessão fechada.",
    "live_copied": "Ligação copiada!",
    "live_json_name": "resultados-live",
    "live_png_name": "nuvem-de-palavras",
    "live_back": "Fechar",
    "live_mode_survey": "Questionário",
    "live_pause": "Pausar",
    "live_resume": "Retomar",
    "live_clear": "Limpar",
    "live_end": "Terminar",
    "live_join_at": "Entra em",
    "live_copy": "Copiar ligação",
    "deck_prev": "Anterior",
    "deck_slide": "Atividade",
    "deck_next": "Seguinte",
    "live_share": "Partilhar",
    "live_need_code": "Introduz um código.",
    "live_notfound": "Sessão não encontrada ou terminada.",
    "live_closed": "As respostas estão fechadas.",
    "live_need_text": "Escreve algo primeiro.",
    "live_sent_mod": "Enviado para aprovação.",
    "live_sent": "Enviado!",
    "live_slow": "Demasiado rápido — espera um momento.",
    "scale_all": "Avalia cada afirmação.",
    "pt_useall": "Usa todos os teus pontos",
    "sv_need_ans": "Escreve pelo menos uma resposta.",
    "sv_thanks": "Obrigado! A tua resposta foi enviada.",
    "live_leave": "Sair",
    "sv_word_ph": "Uma palavra ou uma frase curta…",
    "live_closed_note": "O anfitrião suspendeu as respostas.",
    "sv_send": "Enviar",
    "live_loading": "A carregar…",
    "live_voted": "Voto contado!",
    "live_qa_ph": "Escreve a tua pergunta ou ideia…",
    "live_send": "Enviar",
    "nps_low": "Nada provável",
    "nps_high": "Extremamente provável",
    "rank_submit": "Enviar ranking",
    "pt_remaining": "Restantes:",
    "live_word_ph": "Escreve uma palavra…",
    "live_words_left": (n)=>`Podes enviar mais ${n} palavra(s).`,
    "live_thanks_words": "Obrigado! Vê a nuvem de palavras abaixo.",
    "live_live_results": "Resultados ao vivo",
    "live_options": "Opções",
    "live_option": "Opção",
    "live_add_option": "Adicionar opção",
    "live_rank_opts": "Opções para ordenar",
    "live_rating_scale": "Tipo de escala",
    "live_statements": "Afirmações",
    "live_statement": "Afirmação",
    "live_add_stmt": "Adicionar afirmação",
    "live_scale_hint": "Cada afirmação é avaliada numa escala de 1 (discordo totalmente) a 5 (concordo totalmente).",
    "live_points_opts": "Opções",
    "live_points_hint": "Cada participante divide 100 pontos por estas opções.",
    "live_qa_mod": "Moderação de perguntas",
    "live_qa_mod_off": "Off (todas visíveis)",
    "live_qa_mod_on": "Exigir aprovação",
    "live_qa_mod_hint": "Com aprovação, as perguntas aparecem ao público só depois de as aprovares.",
    "live_filter": "Filtro de palavrões",
    "live_filter_off": "Off",
    "live_filter_on": "On",
    "live_filter_hint": "Mascara automaticamente as palavras vulgares nas respostas (substitui-as por ***).",
    "live_max_words": "Quantas palavras uma pessoa pode enviar",
    "live_mode_single": "Atividade única",
    "live_mode_deck": "Apresentação",
    "live_host_login": "Inicia sessão como administrador para dinamizar uma sessão.",
    "deck_empty": "Ainda não há atividades. Escolhe um tipo acima e adiciona uma.",
    "deck_title": "Título da apresentação",
    "deck_title_ph": "ex. Workshop Atlantykron — comentários",
    "deck_slides": "Atividades",
    "deck_add_new": "Adicionar uma atividade",
    "live_prompt": "Pergunta / tema",
    ...
    "sv_title": "Título do questionário",
    "sv_title_ph": "Ex: Feedback evento ASFAN",
    ...
    "live_join_code": "Entra com um código",
    "live_code_ph": "CÓDIGO",
    "live_join": "Entrar",
    "live_host_new": "Ou inicia uma nova atividade",
    "deck_imported": (n)=>`${n} atividades importadas.`,
    "deck_full": "Máximo 50 atividades.",
    "deck_added": "Atividade adicionada.",
    "deck_need_slides": "Adiciona pelo menos uma atividade.",
    "deck_default_title": "Apresentação",
    "deck_import_bad": "Ficheiro inválido.",
    "deck_import_empty": "Nenhuma atividade válida no ficheiro.",
    "game_waiting_players": "À espera de jogadores… partilha o código!",
    "game_players": "jogadores",
    "game_start": "Começar jogo",
    "gh_typing": "Os participantes estão a escrever…",
    "game_reveal": "Revelar resposta",
    "gh_gotit": "corretas",
    "game_results": "Resultados",
    "game_next": "Seguinte",
    "team_standings": "Classificação por equipas",
    "game_replay": "Jogar de novo",
    "game_report": "Relatório",
    "game_need_name": "Introduz uma alcunha.",
    "game_lobby_closed": "O jogo já começou.",
    "game_times_up": "Tempo esgotado!",
    "game_pick_name": "Escolhe nome e avatar",
    "game_nickname": "A tua alcunha",
    "nick_gen": "Alcunha aleatória",
    "team_pick": "Escolhe a tua equipa",
    "game_enter": "Entrar no jogo",
    "game_youre_in": "Estás dentro!",
    "game_look_screen": "Olha para o ecrã principal.",
    "game_locked": "Resposta enviada!",
    "game_wait_others": "À espera dos outros jogadores…",
    "game_correct": "Correto!",
    "game_wrong": "Errado",
    "game_missed": "Sem resposta",
    "game_your_rank": "A tua posição:",
    "game_finished": "Jogo terminado!",
    "report_back": "Voltar",
    "report_no_data": "Ainda não há dados.",
    "report_players": "jogadores",
    "report_questions": "perguntas",
    "report_avg_score": "pontuação média",
    "report_accuracy": "precisão",
    "report_avg_time": "tempo médio",
    "report_easy": "fácil",
    "report_med": "média",
    "report_hard": "difícil",
    "report_correct": "corretas",
    "report_answered": "respondido",
    "report_player": "Jogador",
    "report_csv": "CSV",
    "report_json": "JSON",
    "report_print": "Imprimir",
    "report_title": "Relatório do jogo",
    "report_byq": "Por pergunta",
    "report_byp": "Por jogador",
    "assign_none_done": "Ainda ninguém terminou.",
    "assign_done": "terminado",
    "assign_joined": "inscritos",
    "assign_closed": "Este trabalho está fechado.",
    "assign_closed_note": "O anfitrião fechou o trabalho.",
    "assign_intro": (n)=>`${n} perguntas · ao teu ritmo`,
    "assign_start": "Começar",
    "assign_finish": "Ver resultado",
    "assign_complete": "Terminaste!",
    "offline_feature": "Indisponível offline.",
    "offline_banner": "Estás offline — solo, biblioteca e editor funcionam. Os modos ao vivo voltam ao reconectar.",
    "install_hint": "Instalar o Undava como app",
    "install_btn": "Instalar",
    "online_back": "Estás de novo online.",
    "live_clear_q": "Eliminar todas as respostas?",
    "live_end_q": "Terminar a sessão para sempre? Os dados serão eliminados."
  },
  "de": {
    "print_quiz": "Drucken",
    "unsaved_q": "Du hast ungespeicherte Änderungen. Ohne Speichern verlassen?",
    "review_btn": "Rückblick",
    "review_title": "Antwort-Rückblick",
    "review_answer": "Richtig:",
    "review_back": "Zurück zur Wertung",
    "sort_by": "Sortieren",
    "sort_default": "Standard",
    "sort_name": "Name A-Z",
    "sort_questions": "Fragenzahl",
    "dup_q": "Frage duplizieren",
    "lib_search": "Quiz suchen…",
    "lib_nomatch": "Keine Quizze gefunden.",
    "tagline": "Quiz ohne Grenzen",
    "set_sound": "Ton",
    "settings": "Einstellungen",
    "spin_need": "Füge mindestens 2 Einträge hinzu.",
    "back_home": "Startseite",
    "spin_title": "Glücksrad",
    "spin_sub": "Gib Namen oder Optionen ein und drehe, um zufällig zu wählen.",
    "spin_go": "Drehen",
    "spin_items": "Einträge",
    "spin_ph": "Ein Eintrag pro Zeile…\nAnna\nBen\nClara",
    "spin_elim": "Gewinner nach jeder Drehung entfernen",
    "spin_shuffle": "Mischen",
    "spin_reset": "Zurücksetzen",
    "spin_default": "Anna|Ben|Clara|David|Eva|Frank",
    "help_book_eli5": "Schnellanleitung",
    "help_book_user": "Benutzerhandbuch",
    "help_book_admin": "Administratorhandbuch",
    "help_title": "Bedienungsanleitung",
    "help_sub": "Vollständiges Handbuch, Schritt für Schritt.",
    ...
    "help_nav": "Anleitung",
    "feat_offline": "100% offline",
    "feat_free": "Handelsfrei",
    "feat_priv": "Keine Telemetrie",
    "feat_open": "Eine Datei",
    "tf_desc": "keine Werbung, keine Datensammlung, kein Konto nötig, keine hinter Bezahlung gesperrten Funktionen",
    "tf_tools": "weitere Werkzeuge",
    ...
    "export": "Exportieren",
    "del": "Löschen",
    "lib_title": "Quiz-Bibliothek",
    "lib_sub": "Wähle ein Quiz zum Spielen oder Bearbeiten",
    "lib_mine": "Meine Quiz",
    "export_all": "Alle exportieren",
    "lib_empty": "Du hast noch keine Quiz erstellt. Tippe auf „Quiz erstellen“, um zu beginnen.",
    "lib_samples": "Beispiel-Quiz",
    "library": "Meine Quiz",
    "new_quiz": "Neues Quiz",
    "edit_quiz": "Quiz bearbeiten",
    "quiz_title": "Quiz-Titel",
    "quiz_title_ph": "z. B. Allgemeinwissen",
    "quiz_desc": "Beschreibung (optional)",
    "quiz_desc_ph": "Eine kurze Beschreibung…",
    "questions": "Fragen",
    "add_question": "Frage hinzufügen",
    "cancel": "Abbrechen",
    "save": "Quiz speichern",
    "acc_primary_ph": "Die richtige Antwort (angezeigt)",
    "acc_alt_ph": "Akzeptierte Variante (Synonym, Schreibweise)",
    "acc_add": "Akzeptierte Variante hinzufügen",
    "acc_hint": "Kleine Tippfehler werden ebenfalls akzeptiert. Die erste Variante wird als richtige Antwort angezeigt.",
    "num_target": "Richtige Zahl",
    "num_target_ph": "z. B. 42",
    "num_tol": "Akzeptierte Toleranz",
    ...
    "question_n": (n)=>"Frage "+n,
    "type_quiz": "Multiple Choice (4)",
    "type_tf": "Wahr / Falsch",
    "type_type": "Freie Antwort",
    "type_num": "Numerisch (raten)",
    "sec": "s",
    "pts_std": "Standard",
    "pts_dbl": "Doppelt",
    "question_ph": "Schreibe hier die Frage…",
    "choose_mode": "Spielmodus",
    "mode_solo": "Solo",
    "mode_solo_d": "Spiele allein und schlage deinen Rekord.",
    "mode_hot": "Reihum",
    "mode_hot_d": "Mehrere Spieler auf demselben Gerät.",
    "shuffle_opt": "Fragen und Antworten mischen",
    "players": "Spieler",
    "add_player": "+ Spieler hinzufügen",
    "start_game": "Spiel starten",
    "your_name": "Dein Name",
    "player_ph": "Spielername",
    "import_copied": "Kopiert!",
    "import_title": "Quiz importieren",
    "import_drop": "Lege eine .json-Datei hierher oder tippe zum Auswählen",
    "import_paste": "…oder füge den JSON-Inhalt ein:",
    "import_paste_ph": "{\"title\":\"…\",\"questions\":[…]}",
    "import_btn": "Importieren",
    "import_fmt_title": "JSON-Format — manuell erstellen",
    "import_fmt_intro": "Du kannst eine .json-Datei mit der Struktur unten schreiben und hier importieren. Hier ein Beispiel mit allen Fragetypen:",
    "import_copy": "Beispiel kopieren",
    "import_template": "Vorlage herunterladen",
    "import_fields": "DATEISTRUKTUR\n• title — der Quizname\n• desc — kurze Beschreibung (optional)\n• color — Hex-Farbe, z. B. #2f6bff (optional)\n• questions — die Liste der Fragen\n\nJEDE FRAGE\n• type — quiz (Auswahl) / tf (wahr-falsch) / type (Textantwort) / num (numerisch)\n• text — die Frage\n• time — Sekunden: 5, 10, 20, 30, 60 oder 120\n• points — 1000 (Standard) oder 2000 (doppelt)\n• answers — die Antworten (siehe unten)\n• tol — nur num: akzeptierte Toleranz (±)\n• explain — optionale Erklärung (warum es richtig ist)\n\nANTWORTEN, NACH TYP\n• quiz: 2–4 Optionen; markiere die richtige mit \"correct\": true\n• tf: 2 Optionen (Wahr / Falsch); markiere die richtige\n• type: 1–6 akzeptierte Texte (alle richtig; füge Schreibvarianten hinzu)\n• num: eine einzige Zahl; tol optional für Toleranz",
    "import_ok": "Quiz importiert!",
    "import_err": "Ungültige Datei. Prüfe das JSON-Format.",
    "you": "Du",
    "need_player": "Füge mindestens einen Spieler hinzu.",
    "scoreboard": "Rangliste",
    "q_of": (a,b)=>"Frage "+a+" von "+b,
    "pass_to": "Gib das Gerät weiter an",
    "tap_start": "Tippe, wenn du bereit bist",
    "ready_q": "Bereit?",
    "num_answer_ph": "Schreibe eine Zahl…",
    "type_answer_ph": "Schreibe deine Antwort…",
    "type_submit": "Senden",
    "keys_hint": "Drücke 1-4 oder tippe eine Antwort",
    "verdict_ok": "Richtig!",
    "verdict_no": "Falsch!",
    "verdict_miss": "Keine Antwort",
    "correct_was": "Richtige Antwort:",
    "you_wrote": "Du hast geschrieben:",
    "streak": (n)=>n+" in Folge 🔥",
    "continue": "Weiter",
    "fastest5": "Die Schnellsten",
    "stat_pts": "Punkte",
    "stat_correct": "Richtige",
    "stat_acc": "Genauigkeit",
    "stat_streak": "Beste Serie",
    "play_again": "Nochmal spielen",
    "podium_title": "Glückwunsch!",
    "winner_is": "Sieger",
    "type_need_answer": "Schreibe eine Antwort.",
    "set_title": "Einstellungen",
    "set_lang": "Sprache",
    "set_lang_d": "Sprache der Oberfläche",
    "set_sound_d": "Soundeffekte während des Spiels",
    ...
    "backup_done": "Backup gespeichert",
    "quit_q": "Spiel verlassen? Der Fortschritt geht verloren.",
    "err_title": "Gib dem Quiz einen Titel.",
    "err_noq": "Füge mindestens eine Frage hinzu.",
    "err_qtext": (n)=>"Frage "+n+" hat keinen Text.",
    "err_acc": (n)=>`Frage ${n}: füge mindestens eine akzeptierte Antwort hinzu.`,
    "err_num": (n)=>`Frage ${n}: gib eine gültige Zahl ein.`,
    "err_ans": (n)=>"Frage "+n+" braucht mindestens 2 Antworten.",
    "err_corr": (n)=>"Markiere die richtige Antwort für Frage "+n+".",
    "explain_ph": "Erklärung (optional) — warum das richtig ist",
    "net_err": "Netzwerkfehler. Prüfe deine Verbindung.",
    "live_t_cloud": "Wortwolke",
    "live_t_cloud_d": "Das Publikum sendet Wörter, live angezeigt.",
    "live_t_poll": "Live-Umfrage",
    "live_t_poll_d": "Über Optionen abstimmen, Balken in Echtzeit.",
    "live_t_qa": "Fragen & Ideen",
    "live_t_qa_d": "Nachrichten des Publikums, mit Stimmen.",
    "live_t_rating": "Bewertung / NPS",
    "live_t_rating_d": "Sternebewertung oder NPS-Wert 0–10.",
    "live_t_rank": "Rangliste",
    "live_t_rank_d": "Die Teilnehmer ordnen die Optionen nach Vorliebe.",
    "live_t_scale": "Skalen (Likert)",
    "live_t_scale_d": "Mehrere Aussagen von 1 bis 5 bewertet (Ablehnung→Zustimmung).",
    "live_t_points": "100 Punkte",
    "live_t_points_d": "Die Teilnehmer verteilen ein Punktebudget auf die Optionen.",
    "cat_quiz": "Quiz & Spiele",
    "cat_quiz_d": "Multiple Choice, wahr/falsch, freie Antwort, numerisch · Solo, reihum, live, Aufgabe oder Teams",
    "cat_audience": "Publikumsaktivitäten",
    "cat_audience_d": "Sammle Live-Antworten vom Publikum, von den Handys. Wähle einen Typ:",
    "cat_deck": "Präsentationen",
    "cat_deck_d": "Verkette mehrere Aktivitäten in einer Sitzung, in deinem Tempo.",
    "deck_build": "Präsentation erstellen",
    "cat_survey": "Feedback-Umfrage",
    "cat_tools": "Werkzeuge",
    "live_nav": "Live",
    "guestbook": "Feedback",
    "mode_live": "Live (Mehrspieler)",
    "mode_live_d": "Leite am großen Bildschirm; das Publikum spielt am Handy.",
    "mode_assign": "Im eigenen Tempo",
    "mode_assign_d": "Weise ein Quiz zu; jeder löst es allein, im eigenen Tempo.",
    "team_opt": "Team-Modus",
    "team_word": "Teams",
    "assign_note": "Die Teilnehmer treten über ihr Handy bei und lösen im eigenen Tempo. Die Ergebnisse sammeln sich hier.",
    "game_join_note": "Die Spieler treten über ihr Handy bei, indem sie den QR-Code scannen oder den angezeigten Code eingeben.",
    "game_host": "Live leiten",
    "assign_host": "Hausaufgabe zuweisen",
    "fb_leave": "Feedback hinterlassen",
    "set_admin": "Administrator",
    "set_admin_on": "Du bist als Administrator angemeldet.",
    "set_admin_off": "Melde dich an, um Quiz zu bearbeiten.",
    "logout": "Abmelden",
    "login": "Anmelden",
    "password": "Passwort",
    "password_again": "Passwort wiederholen",
    "fb_need_rating": "Wähle eine Bewertung (1–5 Sterne).",
    "fb_need_msg": "Schreibe eine Nachricht.",
    "fb_thanks_mod": "Danke! Deine Nachricht erscheint nach der Freigabe.",
    "fb_thanks": "Danke für dein Feedback!",
    "err_pw": "Gib das Passwort ein.",
    "admin_ok": "Angemeldet!",
    "admin_bad": "Falsches Passwort.",
    "err_pw_short": "Das Passwort muss mindestens 6 Zeichen haben.",
    "err_pw_match": "Die Passwörter stimmen nicht überein.",
    "admin_out": "Abgemeldet.",
    "fb_empty": "Noch kein Feedback. Sei der Erste!",
    "fb_title": "Besucher-Feedback",
    "fb_sub": "Hinterlasse einen Gedanken zu den Quiz und sieh, was andere geschrieben haben.",
    "fb_recent": "Neueste Rückmeldungen",
    "fb_local_note": "(lokal gespeichert, auf diesem Gerät)",
    "fb_pick_quiz": "— wähle ein Quiz —",
    "fb_name": "Name (optional)",
    "fb_name_ph": "Wie sollen wir dich nennen?",
    "fb_quiz": "Quiz (optional)",
    "fb_rating": "Bewertung",
    "fb_msg": "Nachricht",
    "fb_msg_ph": "Was hat dir gefallen? Was könnte besser sein?",
    "fb_send": "Feedback senden",
    "fb_anon": "Anonym",
    "fb_pending": "ausstehend",
    "fb_approve": "Freigeben",
    "qa_pending": "Ausstehend",
    "qa_answered": "Beantwortet",
    "qa_approve": "Freigeben",
    "qa_star": "Hervorheben",
    "qa_mark": "Als beantwortet markieren",
    "live_show": "Anzeigen",
    "live_hide": "Ausblenden",
    "qa_pending_count": (n)=>`${n} ausstehend`,
    "live_waiting": "Warte auf Antworten…",
    "live_responses": "Antworten",
    "rt_prom": "Fürsprecher",
    "rt_pass": "Passive",
    "rt_det": "Kritiker",
    "rt_avg": "Durchschnitt",
    "rk_foot": "nach Durchschnittsrang sortiert (niedriger = besser)",
    "scale_lo": "starke Ablehnung",
    "scale_hi": "starke Zustimmung",
    "pt_budget_each": (n)=>`je ${n} Punkte`,
    "live_open": "OFFEN",
    "live_closed_b": "GESCHLOSSEN",
    "live_need_prompt": "Schreibe eine Frage oder ein Thema.",
    "live_need_opts": "Füge mindestens 2 Optionen hinzu.",
    "live_need_stmts": "Füge mindestens 2 Aussagen hinzu.",
    "sv_need_q": "Schreibe mindestens eine Frage.",
    "live_ended": "Sitzung geschlossen.",
    "live_copied": "Link kopiert!",
    "live_json_name": "live-ergebnisse",
    "live_png_name": "wortwolke",
    "live_back": "Schließen",
    "live_mode_survey": "Umfrage",
    "live_pause": "Pause",
    "live_resume": "Fortsetzen",
    "live_clear": "Löschen",
    "live_end": "Beenden",
    "live_join_at": "Beitreten unter",
    "live_copy": "Link kopieren",
    "deck_prev": "Zurück",
    "deck_slide": "Aktivität",
    "deck_next": "Weiter",
    "live_share": "Teilen",
    "live_need_code": "Gib einen Code ein.",
    "live_notfound": "Sitzung nicht gefunden oder beendet.",
    "live_closed": "Die Antworten sind geschlossen.",
    "live_need_text": "Schreibe zuerst etwas.",
    "live_sent_mod": "Zur Freigabe gesendet.",
    "live_sent": "Gesendet!",
    "live_slow": "Zu schnell — warte einen Moment.",
    "scale_all": "Bewerte jede Aussage.",
    "pt_useall": "Nutze alle deine Punkte",
    "sv_need_ans": "Schreibe mindestens eine Antwort.",
    "sv_thanks": "Danke! Deine Antwort wurde gesendet.",
    "live_leave": "Verlassen",
    "sv_word_ph": "Ein Wort oder eine kurze Phrase…",
    "live_closed_note": "Der Gastgeber hat die Antworten pausiert.",
    "sv_send": "Senden",
    "live_loading": "Lädt…",
    "live_voted": "Stimme gezählt!",
    "live_qa_ph": "Schreibe deine Frage oder Idee…",
    "live_send": "Senden",
    "nps_low": "Überhaupt nicht wahrscheinlich",
    "nps_high": "Äußerst wahrscheinlich",
    "rank_submit": "Reihenfolge senden",
    "pt_remaining": "Verbleibend:",
    "live_word_ph": "Schreibe ein Wort…",
    "live_words_left": (n)=>`Du kannst noch ${n} Wort/Wörter senden.`,
    "live_thanks_words": "Danke! Sieh dir die Wortwolke unten an.",
    "live_live_results": "Live-Ergebnisse",
    "live_options": "Optionen",
    "live_option": "Option",
    "live_add_option": "Option hinzufügen",
    "live_rank_opts": "Zu ordnende Optionen",
    "live_rating_scale": "Skalentyp",
    "live_statements": "Aussagen",
    "live_statement": "Aussage",
    "live_add_stmt": "Aussage hinzufügen",
    "live_scale_hint": "Jede Aussage wird auf einer Skala von 1 (starke Ablehnung) bis 5 (starke Zustimmung) bewertet.",
    "live_points_opts": "Optionen",
    "live_points_hint": "Jeder Teilnehmer verteilt 100 Punkte auf diese Optionen.",
    "live_qa_mod": "Fragen-Moderation",
    "live_qa_mod_off": "Aus (alle sichtbar)",
    "live_qa_mod_on": "Freigabe verlangen",
    "live_qa_mod_hint": "Mit Freigabe erscheinen die Fragen dem Publikum erst, nachdem du sie freigegeben hast.",
    "live_filter": "Schimpfwortfilter",
    "live_filter_off": "Aus",
    "live_filter_on": "An",
    "live_filter_hint": "Maskiert automatisch vulgäre Wörter in Antworten (ersetzt sie durch ***).",
    "live_max_words": "Wie viele Wörter eine Person senden kann",
    "live_mode_single": "Einzelne Aktivität",
    "live_mode_deck": "Präsentation",
    "live_host_login": "Melde dich als Administrator an, um eine Sitzung zu leiten.",
    "deck_empty": "Noch keine Aktivitäten. Wähle oben einen Typ und füge eine hinzu.",
    "deck_title": "Titel der Präsentation",
    "deck_title_ph": "z. B. Atlantykron-Workshop — Rückmeldungen",
    "deck_slides": "Aktivitäten",
    "deck_add_new": "Aktivität hinzufügen",
    "live_prompt": "Frage / Thema",
    ...
    "sv_title": "Umfragetitel",
    "sv_title_ph": "z. B. ASFAN-Event-Feedback",
    ...
    "live_join_code": "Mit Code beitreten",
    "live_code_ph": "CODE",
    "live_join": "Beitreten",
    "live_host_new": "Oder starte eine neue Aktivität",
    "deck_imported": (n)=>`${n} Aktivitäten importiert.`,
    "deck_full": "Maximal 50 Aktivitäten.",
    "deck_added": "Aktivität hinzugefügt.",
    "deck_need_slides": "Füge mindestens eine Aktivität hinzu.",
    "deck_default_title": "Präsentation",
    "deck_import_bad": "Ungültige Datei.",
    "deck_import_empty": "Keine gültigen Aktivitäten in der Datei.",
    "game_waiting_players": "Warte auf Spieler… teile den Code!",
    "game_players": "Spieler",
    "game_start": "Spiel starten",
    "gh_typing": "Teilnehmer tippen…",
    "game_reveal": "Antwort zeigen",
    "gh_gotit": "richtig",
    "game_results": "Ergebnisse",
    "game_next": "Weiter",
    "team_standings": "Team-Rangliste",
    "game_replay": "Nochmal spielen",
    "game_report": "Bericht",
    "game_need_name": "Gib einen Spitznamen ein.",
    "game_lobby_closed": "Das Spiel hat bereits begonnen.",
    "game_times_up": "Zeit abgelaufen!",
    "game_pick_name": "Wähle Name und Avatar",
    "game_nickname": "Dein Spitzname",
    "nick_gen": "Zufälliger Spitzname",
    "team_pick": "Wähle dein Team",
    "game_enter": "Spiel betreten",
    "game_youre_in": "Du bist dabei!",
    "game_look_screen": "Schau auf den Hauptbildschirm.",
    "game_locked": "Antwort gesendet!",
    "game_wait_others": "Warte auf die anderen Spieler…",
    "game_correct": "Richtig!",
    "game_wrong": "Falsch",
    "game_missed": "Keine Antwort",
    "game_your_rank": "Dein Platz:",
    "game_finished": "Spiel vorbei!",
    "report_back": "Zurück",
    "report_no_data": "Noch keine Daten.",
    "report_players": "Spieler",
    "report_questions": "Fragen",
    "report_avg_score": "Durchschnittspunktzahl",
    "report_accuracy": "Genauigkeit",
    "report_avg_time": "Durchschnittszeit",
    "report_easy": "leicht",
    "report_med": "mittel",
    "report_hard": "schwer",
    "report_correct": "richtig",
    "report_answered": "beantwortet",
    "report_player": "Spieler",
    "report_csv": "CSV",
    "report_json": "JSON",
    "report_print": "Drucken",
    "report_title": "Spielbericht",
    "report_byq": "Nach Frage",
    "report_byp": "Nach Spieler",
    "assign_none_done": "Noch niemand ist fertig.",
    "assign_done": "fertig",
    "assign_joined": "beigetreten",
    "assign_closed": "Diese Aufgabe ist geschlossen.",
    "assign_closed_note": "Der Gastgeber hat die Aufgabe geschlossen.",
    "assign_intro": (n)=>`${n} Fragen · im eigenen Tempo`,
    "assign_start": "Starten",
    "assign_finish": "Ergebnis ansehen",
    "assign_complete": "Du bist fertig!",
    "offline_feature": "Offline nicht verfügbar.",
    ...
    "install_hint": "Undava als App installieren",
    "install_btn": "Installieren",
    "online_back": "Du bist wieder online.",
    "live_clear_q": "Alle Antworten löschen?",
    "live_end_q": "Die Sitzung endgültig beenden? Die Daten werden gelöscht."
  }
};
...
let libSort="default"; let libSearchQ="";
let editorSnapshot=null; let editorSnapRef=null;
...
function safeColor(c){ return (typeof c==="string" && /^#[0-9a-fA-F]{3,8}$/.test(c)) ? c : "#2f6bff"; }
...
function toast(msg){
  ...
  const el=document.createElement("div"); el.className="toast"; el.setAttribute("role","status"); el.setAttribute("aria-live","polite"); el.textContent=msg;
...
function renderTopBar(){
  ...
  tb.innerHTML=`
    ...
    <div class="tools">
      <button class="iconbtn" data-action="sound" aria-label="${esc(t("set_sound"))}" title="${esc(t("set_sound"))}">${state.sound?"🔊":"🔇"}</button>
      <button class="iconbtn" data-action="settings" aria-label="${esc(t("settings"))}" title="${esc(t("settings"))}">⚙️</button>
    </div>`;
}
...
function viewHome(){
  ...
  return `<div class="wrap">
    ...
    <footer class="tf-foot">
      ...
      <div class="tf-ver">Undava v${APP_VERSION}</div>
    </footer>
  </div>`;
}
...
function quizCard(q){
  ...
  return `<div class="qcard" data-search="${esc((L(q.title)+" "+L(q.desc)).toLowerCase())}">
    <div class="top" style="background:linear-gradient(135deg,${safeColor(q.color)},rgba(0,0,0,.25))">
...
function viewLibrary(){
  ...
  return `<div class="wrap">
    ...
    <div style="display:flex;gap:10px;margin:0 0 12px;flex-wrap:wrap">
      <input class="input" id="libsearch" style="flex:1;min-width:180px;margin:0" placeholder="${esc(t("lib_search"))}" aria-label="${esc(t("lib_search"))}">
      <select class="select" id="libsort" style="width:auto;margin:0" aria-label="${esc(t("sort_by"))}"><option value="default">${esc(t("sort_default"))}</option><option value="name">${esc(t("sort_name"))}</option><option value="questions">${esc(t("sort_questions"))}</option></select>
    </div>
    <div id="libnomatch" style="display:none;color:var(--muted);text-align:center;padding:14px 0 4px">${esc(t("lib_nomatch"))}</div>
    <div class="page-head" style="margin-bottom:12px"><h2 style="font-size:20px">${esc(t("lib_mine"))}</h2>${mine.length?`<button class="btn btn-ghost sm" data-action="exportall" aria-label="${esc(t("export_all"))}" title="${esc(t("export_all"))}">⬇ ${esc(t("export_all"))}</button>`:""}</div>
    <div class="grid">${ mine.length? sortQuizzes(mine).map(quizCard).join("") : `<div class="card-empty">${esc(t("lib_empty"))}</div>` }</div>
...
function toEditable(q){
  ...
  clone.questions=clone.questions.map(qq=>({
    ...
    answers:qq.answers.map(a=>({text:L(a.text), correct:!!a.correct})), tol:qq.tol||0, explain:(qq.explain!=null?L(qq.explain):"")
  }));
  return clone;
}
...
function questionEditor(qq,i,times){
  ...
  return `<div class="qedit" data-qi="${i}">
    <div class="qhead">
      ...
      <button class="iconbtn" style="width:38px;height:38px" data-action="dupq" data-q="${i}" title="${esc(t("dup_q"))}">⧉</button>
    ...
    <div class="qbody">
      ...
      ${body}
      <div class="field" style="margin-top:12px"><input class="input q-explain" data-q="${i}" placeholder="${esc(t("explain_ph"))}" value="${esc(qq.explain||"")}" maxlength="200"></div>
    </div>
  </div>`;
}

function bindEditor(){
  if(state.editing && state.editing!==editorSnapRef){ editorSnapRef=state.editing; editorSnapshot=JSON.stringify(state.editing); }
  ...
  root.querySelectorAll(".q-explain").forEach(el=>el.oninput=e=>{ q.questions[+e.target.dataset.q].explain=e.target.value; });
...
function viewSetup(){
  ...
  return `<div class="wrap">
    ...
    <button class="btn btn-ghost btn-block" data-action="printquiz" style="margin-top:10px">🖨 ${esc(t("print_quiz"))}</button>
  </div>`;
}
...
const EXAMPLE_JSON = `{
  ...
  "questions": [
    { "type": "quiz", "text": "2 + 2 = ?", "time": 20, "points": 1000,
      "answers": [ {"text":"4","correct":true}, {"text":"3"}, {"text":"5"}, {"text":"22"} ], "explain": "2 + 2 = 4" },
...
function sortQuizzes(arr){
  const a=[...arr];
  if(libSort==="name") a.sort((x,y)=>L(x.title).localeCompare(L(y.title)));
  else if(libSort==="questions") a.sort((x,y)=>y.questions.length-x.questions.length);
  return a;
}
function filterLibrary(qstr){
  const q=(qstr||"").toLowerCase().trim(); let visible=0;
  document.querySelectorAll(".qcard[data-search]").forEach(c=>{ const show=!q||(c.getAttribute("data-search")||"").indexOf(q)>=0; c.style.display=show?"":"none"; if(show)visible++; });
  const nm=document.getElementById("libnomatch"); if(nm) nm.style.display=(q&&visible===0)?"":"none";
}
function bindLibrary(){
  const s=document.getElementById("libsearch");
  if(s){ s.value=libSearchQ; s.oninput=()=>{ libSearchQ=s.value; filterLibrary(s.value); }; filterLibrary(libSearchQ); }
  const so=document.getElementById("libsort"); if(so){ so.value=libSort; so.onchange=()=>{ libSort=so.value; render(); }; }
}
function bindImport(){
...
function sanitizeImported(raw){
  ...
  const q={ id:uid(), color:safeColor(raw.color), title:cap(raw.title,80)||"Quiz", desc:cap(raw.desc,140), questions:[] };
  raw.questions.forEach(qq=>{
    ...
    let ans, tol=0;
    if(type==="type"){ ans=qq.answers.slice(0,6).map(a=>({text:cap(a&&a.text,90),correct:true})).filter(a=>String(a.text).trim()); if(ans.length<1) return; }
    else if(type==="num"){ const tgt=(qq.answers[0]&&qq.answers[0].text!=null)?String(qq.answers[0].text):""; if(!isFinite(parseFloat(tgt.replace(",",".")))) return; ans=[{text:tgt.trim().slice(0,20),correct:true}]; tol=Math.abs(parseFloat(String(qq.tol||"0").replace(",","."))||0); }
    else { if(qq.answers.length<2) return; ans=qq.answers.slice(0,4).map(a=>({text:cap(a&&a.text,90), correct:!!(a&&a.correct)})); if(!ans.some(a=>a.correct)) ans[0].correct=true; }
    q.questions.push({ text:cap(qq.text,200), time:[5,10,20,30,60,120].includes(qq.time)?qq.time:20,
      points:qq.points===2000?2000:1000, type:type, answers:ans, tol:tol, explain:(typeof qq.explain==="string")?qq.explain.slice(0,200):"" });
  });
...
function revealView(){
  ...
  if(p.mode==="solo"){
    ...
    return `<div class="reveal">
      ...
      ${q.explain?`<div class="reveal-explain">💡 ${esc(L(q.explain))}</div>`:""}
...
function correctAnswerText(q){
  if(q.type==="num"){ const t0=(q.answers&&q.answers[0])?L(q.answers[0].text):""; return t0+((q.tol&&+q.tol>0)?" ±"+q.tol:""); }
  if(q.type==="type"){ return (q.answers||[]).map(a=>L(a.text)).filter(Boolean).join(" / "); }
  return (q.answers||[]).filter(a=>a.correct).map(a=>L(a.text)).filter(Boolean).join(", ");
}
function reviewView(){
  const p=state.play; const me=p.participants[0]; const qs=p.quiz.questions;
  const rows=qs.map((q,i)=>{
    const ok=!!(me.history[i]&&me.history[i].correct);
    return `<div class="rv-row ${ok?"ok":"no"}">
      <div class="rv-mark">${ok?"✓":"✗"}</div>
      <div class="rv-body"><div class="rv-q">${i+1}. ${esc(L(q.text))}</div>
      <div class="rv-a">${esc(t("review_answer"))} <b>${esc(correctAnswerText(q))}</b></div>${q.explain?`<div class="rv-explain">💡 ${esc(L(q.explain))}</div>`:""}</div>
    </div>`;
  }).join("");
  const correct=me.history.filter(h=>h&&h.correct).length;
  return `<div class="wrap review-screen">
    <div class="page-head"><div><h2>${esc(t("review_title"))}</h2><div class="sub">${correct}/${qs.length} · ${esc(L(p.quiz.title))}</div></div></div>
    <div class="rv-list">${rows}</div>
    <div class="cta-row" style="margin-top:20px">
      <button class="btn btn-primary btn-lg" data-action="reviewback">← ${esc(t("review_back"))}</button>
      <button class="btn btn-ghost btn-lg" data-action="gohome">${esc(t("back_home"))}</button>
    </div>
  </div>`;
}
function printableQuizHTML(quiz){
  const rows=quiz.questions.map((q,i)=>{
    let body;
    if(q.type==="quiz"||q.type==="tf"){
      body="<ol class='pq-opts'>"+q.answers.map(a=>`<li>${a.correct?"<b>✓ </b>":""}${esc(L(a.text))}</li>`).join("")+"</ol>";
    } else {
      body=`<div class='pq-ans'>${esc(t("review_answer"))} <b>${esc(correctAnswerText(q))}</b></div>`;
    }
    return `<div class='pq-q'><div class='pq-qh'>${i+1}. ${esc(L(q.text))}</div>${body}${q.explain?`<div class='pq-explain'>💡 ${esc(L(q.explain))}</div>`:""}</div>`;
  }).join("");
  const d=L(quiz.desc);
  return `<div class='pq-doc'><h1>${esc(L(quiz.title))}</h1>${d?`<div class='pq-desc'>${esc(d)}</div>`:""}<div class='pq-meta'>${esc(t("q_count",quiz.questions.length))}</div>${rows}</div>`;
}
function podiumView(){
  const p=state.play;
  const sorted=[...p.participants].sort((a,b)=>b.score-a.score);
  if(p.mode==="solo"){
    ...
    return `<div class="podium-screen">
      ...
      <div class="cta-row">
        <button class="btn btn-ghost btn-lg" data-action="review">📋 ${esc(t("review_btn"))}</button>
...
document.addEventListener("click",(e)=>{
  ...
  if(state.screen==="editor" && (a==="home"||a==="library") && state.editing && editorSnapshot!==null && JSON.stringify(state.editing)!==editorSnapshot && !confirm(t("unsaved_q"))) return;
  ...
  switch(a){
    ...
    case "exportall": { const mine=state.quizzes.filter(q=>!q.sample); if(!mine.length){ toast(t("lib_empty")); break; } dlText("undava-quizzes-backup.json", JSON.stringify(mine,null,2)); toast(t("backup_done")); break; }
    ...
    case "dupq": { const i=+el.dataset.q; const arr=state.editing.questions; if(arr.length<100){ arr.splice(i+1,0,JSON.parse(JSON.stringify(arr[i]))); rerenderQList(); } break; }
    ...
    case "review": if(state.play){ state.play.phase="review"; window.scrollTo(0,0); render(); } break;
    case "reviewback": if(state.play){ state.play.phase="podium"; render(); } break;
    case "printquiz": { const pq=state.setup&&state.setup.quiz; if(pq){ const pr=document.getElementById("printroot"); if(pr){ pr.innerHTML=printableQuizHTML(pq); window.print(); } } break; }
...
async function saveQuizFromEditor(){
  ...
  const clean={ id:q.__src||q.id, color:safeColor(q.color), title:q.title.trim(), desc:(q.desc||"").trim(),
...
initPWA();
window.addEventListener("beforeunload",(e)=>{ if(state.screen==="editor" && state.editing && editorSnapshot!==null && JSON.stringify(state.editing)!==editorSnapshot){ e.preventDefault(); e.returnValue=""; } });
