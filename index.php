<?php
/* Undava — offline edition, no dependencies (no data/ folder, no server writes).
   Everything runs client-side in the browser (localStorage), exactly like the .html file.
   The HTML below is emitted verbatim from a NOWDOC, so the "<?php" string inside the
   app's JavaScript is treated as plain text and never executed. */
header('Content-Type: text/html; charset=UTF-8');
echo <<<'UNDAVA_OFFLINE_HTML_EOF'
<!DOCTYPE html>
<html lang="ro">
<head>
<meta charset="UTF-8">
...
<meta property="og:site_name" content="Undava">
<meta property="og:title" content="Undava — Quiz fără frontiere">
<meta property="og:description" content="Joc de quiz în spiritul Kahoot — creează întrebări și joacă cu prietenii pe același dispozitiv. Offline, gratuit, fără conturi, fără reclame. Disponibil în 7 limbi: RO · EN · FR · IT · ES · PT · DE.">
<meta property="og:url" content="https://chiuta.github.io/Undava/">
<meta property="og:image" content="https://chiuta.github.io/Undava/undava-og.jpg">
<meta property="og:image:type" content="image/jpeg">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta property="og:image:alt" content="Undava — Quiz fără frontiere">
...
<meta name="twitter:title" content="Undava — Quiz fără frontiere">
<meta name="twitter:description" content="Joc de quiz în spiritul Kahoot — offline, gratuit, fără conturi, fără reclame. În 7 limbi.">
<meta name="twitter:image" content="https://chiuta.github.io/Undava/undava-og.jpg">
<meta name="twitter:image:alt" content="Undava — Quiz fără frontiere">
<!--
...
.gh-fast{margin-top:14px;width:100%;max-width:520px;background:rgba(0,0,0,.16);border:1px solid var(--line);border-radius:var(--r);padding:12px 14px}
.ghf-title{font-weight:800;font-size:13px;letter-spacing:.04em;color:var(--accent);margin-bottom:10px;text-transform:uppercase;text-align:center}
.ghf-row{display:flex;flex-wrap:wrap;gap:8px;justify-content:center}
.ghf-card{flex:1 1 150px;min-width:140px;display:flex;align-items:center;gap:8px;background:rgba(255,255,255,.05);border:1px solid var(--line);border-left:4px solid var(--muted);border-radius:var(--r-sm);padding:7px 10px}
.ghf-card.ok{border-left-color:var(--good)}
.ghf-card.no{border-left-color:var(--bad)}
.ghf-rk{font-weight:900;font-size:15px;width:18px;text-align:center;color:var(--accent);flex:none}
.ghf-av{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:#16092e;flex:none}
...
.acc-ic{flex:none;width:30px;height:30px;border-radius:9px;display:grid;place-items:center;font-weight:900;background:rgba(46,230,196,.14);color:var(--teal,#2ee6c4)}
...
.num-pm{font-size:22px;font-weight:900;color:var(--teal,#2ee6c4)}
...
.type-input:focus{border-color:var(--teal,#2ee6c4)}
...
/* ---------- shuffle toggle ---------- */
.setup-shuffle{display:flex;align-items:center;gap:9px;font-size:14px;font-weight:700;cursor:pointer;margin:0 0 16px;color:var(--ink,#f4f2ff)}
.setup-shuffle input{width:18px;height:18px;accent-color:var(--accent,#7c5cff)}
/* ---------- spinner wheel ---------- */
...
.spin-result b{color:var(--accent,#7c5cff)}
.spin-result-ph{min-height:34px}
.spin-need{display:grid;place-items:center;width:100%;height:100%;border:2px dashed var(--line,#2a2350);border-radius:50%;color:var(--muted);text-align:center;padding:20px}
...
const I18N = {
  "ro": {
    ...
    "server_edition": "Ediția server (live)",
    "php_guide_title": "🖥️ Ediția server necesită PHP",
    "php_guide_intro": "Ediția server (index.php) are nevoie de PHP ca să ruleze — nu se deschide cu dublu-clic. Pune index.php în același folder cu acest fișier, apoi pornește un server:",
    "php_guide_quick": "Test rapid (local) — deschide un terminal în folder și rulează:",
    "php_guide_open": "apoi deschide în browser:",
    "php_guide_real": "Server real (Apache / Nginx + PHP 8.0+): vezi Manualul de administrator din ediția server pentru pașii detaliați.",
    "php_guide_req": "Necesită PHP 8.0 sau mai nou."
  },
  "en": {
    "print_quiz": "Print",
    ...
    "server_edition": "Server edition (live)",
    "php_guide_title": "🖥️ The server edition needs PHP",
    "php_guide_intro": "The server edition (index.php) needs PHP to run — it doesn't open with a double-click. Put index.php in the same folder as this file, then start a server:",
    "php_guide_quick": "Quick test (local) — open a terminal in the folder and run:",
    "php_guide_open": "then open in the browser:",
    "php_guide_real": "Real server (Apache / Nginx + PHP 8.0+): see the Administrator manual in the server edition for detailed steps.",
    "php_guide_req": "Requires PHP 8.0 or newer."
  },
  "fr": {
    "print_quiz": "Imprimer",
    ...
    "server_edition": "Édition serveur (live)",
    "php_guide_title": "🖥️ L'édition serveur nécessite PHP",
    "php_guide_intro": "L'édition serveur (index.php) a besoin de PHP pour fonctionner — elle ne s'ouvre pas d'un double-clic. Place index.php dans le même dossier que ce fichier, puis démarre un serveur :",
    "php_guide_quick": "Test rapide (local) — ouvre un terminal dans le dossier et exécute :",
    "php_guide_open": "puis ouvre dans le navigateur :",
    "php_guide_real": "Serveur réel (Apache / Nginx + PHP 8.0+) : voir le Manuel de l'administrateur dans l'édition serveur pour les étapes détaillées.",
    "php_guide_req": "Nécessite PHP 8.0 ou plus récent."
  },
  "it": {
    "print_quiz": "Stampa",
    ...
    "server_edition": "Edizione server (dal vivo)",
    "php_guide_title": "🖥️ L'edizione server richiede PHP",
    "php_guide_intro": "L'edizione server (index.php) ha bisogno di PHP per funzionare — non si apre con un doppio clic. Metti index.php nella stessa cartella di questo file, poi avvia un server:",
    "php_guide_quick": "Test rapido (locale) — apri un terminale nella cartella ed esegui:",
    "php_guide_open": "poi apri nel browser:",
    "php_guide_real": "Server reale (Apache / Nginx + PHP 8.0+): vedi il Manuale dell'amministratore nell'edizione server per i passaggi dettagliati.",
    "php_guide_req": "Richiede PHP 8.0 o più recente."
  },
  "es": {
    "print_quiz": "Imprimir",
    ...
    "server_edition": "Edición servidor (en directo)",
    "php_guide_title": "🖥️ La edición servidor necesita PHP",
    "php_guide_intro": "La edición servidor (index.php) necesita PHP para funcionar — no se abre con doble clic. Pon index.php en la misma carpeta que este archivo, luego inicia un servidor:",
    "php_guide_quick": "Prueba rápida (local) — abre un terminal en la carpeta y ejecuta:",
    "php_guide_open": "luego abre en el navegador:",
    "php_guide_real": "Servidor real (Apache / Nginx + PHP 8.0+): consulta el Manual del administrador en la edición servidor para los pasos detallados.",
    "php_guide_req": "Requiere PHP 8.0 o más reciente."
  },
  "pt": {
    "print_quiz": "Imprimir",
    ...
    "server_edition": "Edição servidor (ao vivo)",
    "php_guide_title": "🖥️ A edição servidor precisa de PHP",
    "php_guide_intro": "A edição servidor (index.php) precisa de PHP para funcionar — não se abre com um duplo clique. Coloca index.php na mesma pasta que este ficheiro, depois inicia um servidor:",
    "php_guide_quick": "Teste rápido (local) — abre um terminal na pasta e executa:",
    "php_guide_open": "depois abre no navegador:",
    "php_guide_real": "Servidor real (Apache / Nginx + PHP 8.0+): vê o Manual do administrador na edição servidor para os passos detalhados.",
    "php_guide_req": "Requer PHP 8.0 ou mais recente."
  },
  "de": {
    "print_quiz": "Drucken",
    ...
    "server_edition": "Server-Edition (live)",
    "php_guide_title": "🖥️ Die Server-Edition benötigt PHP",
    "php_guide_intro": "Die Server-Edition (index.php) benötigt PHP zum Ausführen — sie öffnet sich nicht per Doppelklick. Lege index.php in denselben Ordner wie diese Datei, dann starte einen Server:",
    "php_guide_quick": "Schnelltest (lokal) — öffne ein Terminal im Ordner und führe aus:",
    "php_guide_open": "dann öffne im Browser:",
    "php_guide_real": "Echter Server (Apache / Nginx + PHP 8.0+): siehe das Administratorhandbuch in der Server-Edition für detaillierte Schritte.",
    "php_guide_req": "Erfordert PHP 8.0 oder neuer."
  }
};
...
const state={ lang:"ro", sound:true, screen:"home", quizzes:[], editing:null, play:null, setup:null, modal:null, importBuf:"", spinner:{items:[],result:null,spinning:false,elim:false}, spinAngle:0 };

function loadStore(){ try{ return JSON.parse(localStorage.getItem(STORE_KEY))||{}; }catch(e){ return {}; } }
function saveStore(){ try{ localStorage.setItem(STORE_KEY, JSON.stringify({ lang:state.lang, sound:state.sound, userQuizzes:state.quizzes.filter(q=>!q.sample) })); }catch(e){} }
function initStore(){
  const s=loadStore();
  if(s.lang) state.lang=s.lang;
  if(typeof s.sound==="boolean") state.sound=s.sound;
  const user=Array.isArray(s.userQuizzes)?s.userQuizzes:[];
  state.quizzes=[...samples(), ...user];
}
  ...
  return `<div class="wrap center-stage">
    <section class="hero">
      ...
      <div class="cta-row">
        ...
        <button class="btn btn-ghost btn-lg" data-action="serveredition">🖥️ ${esc(t("server_edition"))}</button>
      </div>
      <div class="feat">
...
function quizCard(q){
  ...
  return `<div class="qcard" data-search="${esc((L(q.title)+" "+L(q.desc)).toLowerCase())}">
    ...
    <div class="actions">
      ...
      <button class="btn btn-ghost mini" data-action="edit" data-id="${q.id}" title="${esc(t("edit"))}">✎</button>
      <button class="btn btn-ghost mini" data-action="dup" data-id="${q.id}" title="${esc(t("duplicate"))}">⧉</button>
      <button class="btn btn-ghost mini" data-action="export" data-id="${q.id}" title="${esc(t("export"))}">⬇</button>
      ${isS?"":`<button class="btn btn-ghost mini" data-action="delquiz" data-id="${q.id}" title="${esc(t("del"))}">🗑</button>`}
    </div>
  </div>`;
}
...
function matchText(input, accepted){ const g=normText(input); if(!g) return false; const list=Array.isArray(accepted)?accepted:[accepted]; for(const acc of list){ const a=normText(acc); if(!a) continue; if(g===a) return true; const allowed=Math.min(2,Math.floor(a.length/4)); if(allowed>0 && levDist(g,a)<=allowed) return true; } return false; }
  ...
  return `<div class="wrap">
    ...
    <div class="panel">
      ...
      <div class="players-list" id="players">
        ${su.players.map((p,i)=>playerRow(p,i,su.mode)).join("")}
      </div>
    ...
    <button class="btn btn-primary btn-lg btn-block" data-action="startgame">🚀 ${esc(t("start_game"))}</button>
...
function tryImport(txt){
  try{
    ...
    let ok=0;
    arr.forEach(raw=>{ const q=sanitizeImported(raw); if(q){ state.quizzes.push(q); ok++; } });
    if(!ok) throw new Error("no valid quiz");
    saveStore();
    toast(t("import_ok"));
...
function bindPlay(){
  ...
  document.onkeydown=(e)=>{
    ...
    const q=curQuestion(); if(q.type==="type"||q.type==="num") return;
    const map={"1":0,"2":1,"3":2,"4":3,"q":0,"w":1,"e":2,"r":3};
    const k=e.key.toLowerCase();
    if(map[k]!=null && map[k]<q.answers.length){ e.preventDefault(); pressAnswer(map[k]); }
  };
  const p=state.play;
  if(p&&p.phase==="question"&&!p.answered){ const q=curQuestion(); if(q&&(q.type==="type"||q.type==="num")){ const inp=document.getElementById("type-input"); if(inp){ inp.focus(); inp.onkeydown=(e)=>{ if(e.key==="Enter"){ e.preventDefault(); const v=(inp.value||"").trim(); if(v) submitTextAnswer(v); else toast(t("type_need_answer")); } }; } } }
}
...
/* ---------------- MODAL (settings) ---------------- */
function openServerEdition(){
  fetch("index.php?asset=manifest",{cache:"no-store"}).then(function(r){ return r.text(); }).then(function(txt){
    var ok=false; try{ var j=JSON.parse(txt); if(j&&(j.name==="Undava"||j.short_name)) ok=true; }catch(e){}
    if(txt.indexOf("<?php")>=0) ok=false;
    if(ok){ location.href="index.php"; } else { state.modal="phpguide"; renderModal(); }
  }).catch(function(){ state.modal="phpguide"; renderModal(); });
}
function renderModal(){
  ...
  if(state.modal==="phpguide"){
    m.innerHTML=`<div class="modal-bg" data-action="closemodal-bg"><div class="modal" data-stop="1">
      <h3>${esc(t("php_guide_title"))}</h3>
      <div class="about" style="margin:0 0 10px">${esc(t("php_guide_intro"))}</div>
      <div style="font-weight:700;font-size:13px;margin:8px 0 4px">${esc(t("php_guide_quick"))}</div>
      <div style="background:rgba(0,0,0,.35);border:1px solid rgba(255,255,255,.18);border-radius:10px;padding:10px 12px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:14px;user-select:all;word-break:break-all">php -S 0.0.0.0:8080</div>
      <div class="about" style="margin:8px 0 2px">${esc(t("php_guide_open"))} <b>http://localhost:8080/</b></div>
      <div class="about" style="margin:10px 0 0">${esc(t("php_guide_real"))}</div>
      <div class="about" style="margin:8px 0 0;opacity:.65">${esc(t("php_guide_req"))}</div>
      <button type="button" class="btn btn-primary btn-block" style="margin-top:16px" data-action="closemodal">${esc(t("close"))}</button>
    </div></div>`;
  }
}
...
document.addEventListener("click",(e)=>{
  ...
  switch(a){
    ...
    case "create": state.editing=Object.assign(blankQuiz(),{__new:true}); state.screen="editor"; render(); break;
    case "edit": {
    ...
    case "dup": {
      ...
      state.quizzes.push(c); saveStore(); state.screen="library"; render(); toast(t("saved")); break;
    }
    case "export": exportQuiz(state.quizzes.find(x=>x.id===id)); break;
    case "delquiz": {
      if(confirm(t("confirm_del"))){ state.quizzes=state.quizzes.filter(x=>x.id!==id); saveStore(); render(); toast(t("deleted")); }
      break;
    }
    case "import": state.screen="import"; render(); break;
    case "serveredition": openServerEdition(); break;
    ...
    case "spinner": if(!state.spinner.items||!state.spinner.items.length) spinLoad(); state.spinner.result=null; state.spinAngle=0; state.screen="spinner"; render(); break;
...
function openSetup(id){
  const q=state.quizzes.find(x=>x.id===id);
  state.setup={ quiz:q, mode:"solo", players:[""], shuffle:false };
...
function replayGame(){
  ...
  stopAll();
  state.setup={quiz, mode, players:names, shuffle:false};
  startGame();
}
...
function saveQuizFromEditor(){
  ...
  for(let i=0;i<q.questions.length;i++){
    ...
    if(qq.type==="tf"){ /* fixed labels */ } else if(qq.type==="type"){ if(valid<1){ toast(t("err_acc",i+1)); return; } } else if(qq.type==="num"){ const tgt=String(qq.answers[0]&&qq.answers[0].text||"").replace(",",".").trim(); if(tgt===""||!isFinite(parseFloat(tgt))){ toast(t("err_num",i+1)); return; } } else if(valid<2){ toast(t("err_ans",i+1)); return; }
    if(qq.type!=="type" && qq.type!=="num" && !qq.answers.some(a=>a.correct)){ toast(t("err_corr",i+1)); return; }
  }
  // normalize answers (tf gets proper labels stored as plain strings)
  ...
  // save: replace if editing existing user quiz, else add
  const existing=state.quizzes.findIndex(x=>x.id===clean.id && !x.sample);
  if(existing>=0) state.quizzes[existing]=clean; else state.quizzes.push(clean);
  saveStore();
  toast(t("saved"));
